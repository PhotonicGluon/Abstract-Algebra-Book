...
Looking back at the polynomial $x^2+1$, we note that both $\Q(i)$ and $\Q[x]/\princ{x^2+1}$ split $x^2+1$. But are these two fields different? Not really; we end this section by proving that a splitting field of a polynomial over a given field is unique up to isomorphism.

But we introduce some terminology and notation first.
...
% \begin{lemma}\label{lemma-isomorphism-extension-lemma}
%     Let $F$ and $F'$ be fields, $p(x) \in F[x]$ be irreducible over $F$, and let $a$ be a zero of $p(x)$ in some field extension $E/F$. If $\phi: F \to F'$ is an isomorphism and $b$ is a zero of $p(x)$ in some field extension $E'/F'$, then there exists an isomorphism $\overline{\phi}: F(a) \to F'(b)$ that agrees with $\phi$ on $F$ and maps $a$ to $b$.
% \end{lemma}
% \begin{proof}[Proof (see {\cite[p.~345]{gallian_2016}} and {\cite[Lemma 21.32]{judson_beezer_2022}})]
%     We note that $F(a) \cong F[a]$ and $F'(b) \cong F'[b]$ by \myref{thrm-simple-extension-isomorphism}. Furthermore, if $\deg p(x) = n$, we have that both $F[a]$ and $F'[b]$ have dimension $n$ with bases
%     \[
%         \{1, a, a^2, \dots, a^n\} \quad\text{and}\quad\{1, b, b^2, \dots, b^n\}
%     \]
%     respectively. Define the isomorphism $\overline{\phi}: F(a) \to F'(b)$ where
%     \[
%         \overline{\phi}(a_0 + a_1)
%     \]

%     For brevity let $q(x) = \phi(p(x))$. Note that $q(x)$ is irreducible over $F'$. Therefore we know that both $\princ{p(x)}$ and $\princ{q(x)}$ are maximal ideals (\myref{thrm-irreducible-iff-principal-ideal-is-maximal}), and so both $F/\princ{p(x)}$ and $F'/\princ{q(x)}$ are fields (\myref{thrm-maximal-ideal-iff-quotient-ring-is-field}). Define the map $\psi: F[x]/\princ{p(x)} \to F'[x]/\princ{q(x)}$ by
%     \[
%         \psi(f(x) + \princ{p(x)}) = \phi(f(x)) + \princ{q(x)}.
%     \]
%     We check that $\psi$ is an isomorphism.
%     \begin{itemize}
%         \item \textbf{Homomorphism}: % TODO: Add
...
%         \item \textbf{Surjective}: % TODO: Add
%     \end{itemize}
%     So $\psi$ is an isomorphism. Also, \myref{thrm-simple-extension-isomorphism} allows us to find the isomorphisms $\alpha: F(a) \to F[x]/\princ{p(x)}$ and $\beta: F'(b) \to F'[x]/\princ{q(x)}$. Hence $\overline{\phi}: F(a) \to F'(b)$ where $\overline{\phi} = \beta^{-1}\psi\alpha$ is an isomorphism (since it is the composition of isomorphisms).
% \end{proof}
