...
\begin{proof}
    ...
    If instead $n = 0$, consider the map $\phi: \Q \to K$ such that $\frac ab \mapsto (a1)(b1)^{-1}$. We show that $\phi$ is a well-defined isomorphism.
    ...
    \begin{itemize}
        ...
        \item \textbf{Injective}: Suppose $\frac ab, \frac cd \in \Q$ such that $\phi\left(\frac ab\right) = \phi\left(\frac cd\right)$. Thus we have $(a1)(b1)^{-1} = (c1)(d1)^{-1}$, meaning $(a1)(d1) = (b1)(c1)$. So $(ad)1 = (bc)1$ which means $(ad - bc)1 = 0$, i.e. $ad - bc = 0$. Therefore $ad = bc$ which means $\frac ab = \frac cd$, i.e. $\phi$ is injective.
        
        \item \textbf{Surjective}: Suppose $n1 \in K$. Note $\phi(\frac n1) = n1$, so $n1$ has a pre-image in $\Q$, i.e. $\phi$ is a surjection.
    \end{itemize}
    Therefore we have shown that $\phi$ is a isomorphism, meaning $K \cong \Q$.
\end{proof}

To end, we look at \textbf{prime fields}.

\begin{definition}
    A \textbf{prime field}\index{field!prime}\index{prime!field} is a field with no proper subfields.
\end{definition}

There is one quite obvious prime field.
...
\begin{exercise}
    Prove or disprove: $\R$ a prime field.
\end{exercise}
