...
\begin{theorem}\label{thrm-homomorphism-from-field-is-isomorphism-or-trivial}
    Let $F$ be a field, $R$ be a ring, and let $\phi: F \to R$ be a ring homomorphism. Then either $\phi$ is an isomorphism or $\phi(x) = 0$ for all $x \in F$.
\end{theorem}
\begin{proof}
    See \myref{problem-homomorphism-from-field-is-isomorphism-or-trivial} (later).
\end{proof}
...
\begin{proof}
    ...
    If instead $n = 0$, consider the map $\phi: \Q \to K$ such that $\frac ab \mapsto (a1)(b1)^{-1}$. We just need to show that $\phi$ is a well-defined homomorphism, as we can use \myref{thrm-homomorphism-from-field-is-isomorphism-or-trivial} to quickly conclude that it is also an isomorphism.
    ...
    \end{itemize}
    Therefore, by \myref{thrm-homomorphism-from-field-is-isomorphism-or-trivial}, we see that $\phi$ is a isomorphism, meaning $K \cong \Q$.
\end{proof}
...
\begin{problem}\label{problem-Q-is-prime-field}
    % TODO: Add Q is a prime field; see https://proofwiki.org/wiki/Rational_Numbers_form_Prime_Field
\end{problem}
