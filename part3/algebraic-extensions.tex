...
\begin{theorem}\label{thrm-multiplying-finite-extension-degrees}
...
The statement of \myref{thrm-multiplying-finite-extension-degrees} can be summarised in the following field extension diagram.
...
\begin{examplewithcutout}{right}{0}{0.6\textwidth}{0pt}{8}{
    ...
    We see that $[\Q(\sqrt3, \sqrt5), \Q(\sqrt5)] = 2$ and $[\Q(\sqrt5):\Q] = 2$, so $[\Q(\sqrt3, \sqrt5):\Q] = 4$ by \myref{thrm-multiplying-finite-extension-degrees}. In particular, one sees that $\{1, \sqrt3\}$ is a basis for $\Q(\sqrt3,\sqrt5)$ over $\Q(\sqrt5)$ and $\{1, \sqrt5\}$ is a basis for $\Q(\sqrt5)$ over $\Q$, so $\{1, \sqrt3, \sqrt5, \sqrt3\times\sqrt5\} = \{1, \sqrt3, \sqrt5, \sqrt{15}\}$ is a basis for $\Q(\sqrt3, \sqrt5)$.
...
\begin{example}
    ...
    \]
    by \myref{thrm-multiplying-finite-extension-degrees}. But $[\Q(\alpha):\Q] = 4$ (as $\deg p(x) = 4$) and $[\Q(\sqrt[3]2):\Q] = 3$, so we see that 3 divides 4, a clear contradiction. Therefore $\sqrt[3]2 \notin \Q(\alpha)$.
\end{example}
...
\begin{proof}
    ...
    \]
    by \myref{thrm-multiplying-finite-extension-degrees}. As $a$ is algebraic over $F$ it is certainly algebraic over $F(b)$. Therefore $[F(b):F]$ is finite (since $b$ is algebraic over $F$) and $[F(a,b):F(b)]$ is finite (since $a$ is algebraic over $F(b)$), meaning $[F(a,b):F]$ is finite. Hence $F(a,b)$ is a finite extension, $F(a,b)$ is an algebraic extension over $F$ (\myref{thrm-finite-extension-is-algebraic}). Therefore $a+b$, $a-b$, $ab$, and $ab^{-1}$ (where $b \neq 0$ in this last case) are all algebraic since $a+b,a-b,ab,ab^{-1} \in F(a,b)$ and $F(a,b)$ is an algebraic extension.
...
\begin{exercise}\label{exercise-algebraically-closed-field-has-no-proper-algebraic-extension}
    Prove \myref{corollary-algebraically-closed-field-has-no-proper-algebraic-extension} (later).
\end{exercise}
