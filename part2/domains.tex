...
\begin{theorem}\label{thrm-field-of-fractions-is-smallest-field-containing-integral-domain}
    Let $D$ be an integral domain. Then $\Frac{D}$ is the smallest field containing a subring isomorphic to $D$. That is, if $F$ is a field that contains a subring isomorphic to $D$, then $F$ has a subring $K$ that is isomorphic to $\Frac{D}$.
\end{theorem}
\begin{proof}
    See \myref{problem-field-of-fractions-is-smallest-field-containing-integral-domain} (later).
\end{proof}
...
\end{corollary}
\begin{proof}
    See \myref{exercise-Z-is-UFD} (later).
\end{proof}
