...
\]
For most choices of $f(x)$ this will be true; if not, just randomly pick another $f(x)$ from $\mathcal{L}_f$. Bob then computes the polynomial $h(x)$ given by
\[
    h(x) = F_q(x)g(x) \mod q,
\]
...
\begin{example}
    ...
    Suppose Alice wants to transmit the character `H' to Bob. Using the conversion process described, Alice converts `H' into the message polynomial $m_1(x) = x^6 + x^3$. Say Alice obtains a random `encoding fuzz' polynomial of $\phi(x) = x^{3} - x^{2} + x - 1$. Hence her encrypted message will be
    \begin{align*}
...
\begin{example}
    Using the same parameters as the previous example, Alice then transmits the character `i' to Bob. Alice converts `i' into the message polynomial $m_2(x) = x^6 + x^5 + x^3 + x^2$, and her encrypted message will be
    \begin{align*}
...
\begin{exercise}
    ...
    \]
    Alice wants to transmit the character `U' to Bob. Suppose Alice encodes `U' using the ASCII conversion table (\myref{table-ASCII-conversion-table}) and obtains an `encoding fuzz' polynomial of $x - 1$. What is the encrypted message polynomial?
\end{exercise}
