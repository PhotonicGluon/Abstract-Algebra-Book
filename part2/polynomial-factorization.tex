...
\section{Criteria for (Ir)reducibility}
...
\subsection{Substitution Rule}  % TODO: Produce a more rigorous proof
To ease checking the irreducibility of polynomials, we come up with the following substitution rule to simplify some polynomials.

\begin{theorem}\label{thrm-substitution-rule-for-irreducibility}
    Let $D$ be an integral domain, $f(x) \in D[x]$, and $a \in D$.
    \begin{enumerate}
        \item $f(x)$ is irreducible if and only if $f(x + a)$ is irreducible.
        \item Suppose $a$ has an inverse. Then $f(x)$ is irreducible if and only if $f(ax)$ is irreducible.
    \end{enumerate} 
\end{theorem}
\begin{proof}
    We prove the contrapositive for each of the statements.
    \begin{enumerate}
        \item We first show the forward direction via contrapositive; suppose $f(x+a)$ is reducible, meaning that there exists non-unit polynomials $p(x)$ and $q(x)$ in $D[x]$ such that $f(x+a) = p(x)q(x)$. Substituting $x$ with $x - a$ we see $f(x) = p(x-a)q(x-a)$.
        \begin{itemize}
            \item If $p(x)$ is a constant polynomial, then it is the same as $p(x-a)$, so $p(x-a)$ is not a unit (since $p(x)$ is assumed to not be a unit). Similar analysis applies to the case when $q(x)$ is a constant polynomial.
            \item Otherwise, $p(x)$ and $q(x)$ are polynomials, which cannot be units by \myref{prop-unit-of-ring-iff-unit-of-polynomial-ring}. Thus $p(x-a)$ and $q(x-a)$ are also polynomials that are not units since both have the same degree as $p(x)$ and $q(x)$ respectively.
        \end{itemize}
        Therefore $f(x)$ is a product of two non-unit polynomials $p(x-a)$ and $q(x-a)$, meaning $f(x)$ is reducible in $D$.

        The reverse direction is readily proven by using $-a$ instead of $a$ in the above proof.

        \item For the forward direction, and working with the contrapositive statement, suppose $f(ax)$ is reducible. So there exist non-unit polynomials $p(x)$ and $q(x)$ in $D[x]$ such that $f(ax) = p(x)q(x)$. As $a$ has an inverse, thus substituting $x$ with $a^{-1}x$ yields $f(x) = p(a^{-1}x)q(a^{-1}x)$. Similar analysis to above also shows that $f(x)$ is reducible.
        
        The reverse direction is also readily proven by using $a^{-1}$ (which exists by hypothesis) in the above proof.
    \end{enumerate}
...
\begin{itemize}
    ...
    \item If $f(x) \in \Z[x]$ is irreducible in $\Z$ then it is irreducible in $\Q$. (\myref{thrm-reducible-over-Q-means-reducible-over-Z})
\end{itemize}
We look at more irreducibility tests, starting with one that is related to \myref{thrm-reducible-over-Q-means-reducible-over-Z}.
...
\begin{example}
    ...
    Since $\bar{F}(x)$ is irreducible, thus $F(x)$ is also irreducible by Mod 2 Irreducibility Test (\myref{thrm-mod-p-irreducibility-test}). By contrapositive of \myref{thrm-reducible-over-Q-means-reducible-over-Z} we know $F(x)$ is irreducible over $\Q$, thus $f(x)$ is irreducible over $\Q$.
\end{example}
...
\end{theorem}
\begin{proof}
    Seeking a contradiction, suppose that $f(x)$ is reducible over $\Q$. By \myref{thrm-reducible-over-Q-means-reducible-over-Z} this means there exist non-constant polynomials $g(x),h(x) \in \Z[x]$ with degree smaller than that of $f(x)$ such that $f(x) = g(x)h(x)$.
