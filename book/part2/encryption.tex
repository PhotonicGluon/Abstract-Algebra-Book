...
\begin{itemize}
    ...
    \item For the rest, first define $\mathcal{L}(d_1, d_2)$ to be the set of polynomials in $R$ with $d_1$ coefficients equal to 1, $d_2$ coefficients equal to -1, and the rest equal to 0. Then, by choosing positive integers $d_f$, $d_g$, and $d_\phi$, set
    \begin{align*}
...
\[
    h(x) = F_q(x)g(x) \mod q,
\]
that is, $h(x)$ is $F_q(x)g(x)$ and then reducing its coefficients modulo $q$. Bob's public key is $h(x)$; his private key is $f(x)$, although in practice he will also store $F_p(x)$.
...
\begin{example}
    Since `H' has a binary code of \code{0100 1000}, we may convert it into a polynomial with 8 terms (possibly with coefficients of zero):
    \[
...
An implementation of NTRU, as described in this chapter, is available on GitHub at \url{https://github.com/PhotonicGluon/Abstract-Algebra-Book} under the \code{misc} directory.
