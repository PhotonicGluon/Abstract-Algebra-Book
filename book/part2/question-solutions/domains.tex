...
\begin{questions}
    ...
    \item Suppose there are only finitely many irreducible polynomials within $F[x]$, and suppose that $p_1(x), p_2(x), \dots, p_n(x)$ is a list of all the irreducible polynomials in $F[x]$. As $F[x]$ is a PID (\myref{thrm-polynomial-ring-over-field-is-a-PID}) it is thus a UFD (\myref{thrm-PID-is-UFD}). Now consider the polynomial
    \[
