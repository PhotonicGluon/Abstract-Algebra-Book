...
\begin{questions}
    ...
    \item We consider the test for ideal (\myref{thrm-test-for-ideal}) to prove that $I\cap J$ is an ideal. We note that as $I$ and $J$ are ideals, they are therefore subrings of $R$. Thus, 0 is in both $I$ and $J$, meaning $0 \in I\cap J$. Hence $I\cap J$ is non-empty.
    ...
    \item We consider the test for ideal (\myref{thrm-test-for-ideal}) to prove $\Ann{R}{A}$ is an ideal of $R$. We note that $\Ann{R}{A}$ is non-empty since $0 \in \Ann{R}{A}$ (because $0a = 0$ for any $a \in A$).
...
\begin{questions}
    ...
    \item \begin{partquestions}{\alph*}
        \item Suppose $r \in \sqrt{\sqrt{I}}$, meaning that $r^m \in \sqrt{I}$ for some positive integer $m$, further meaning that $(r^m)^n \in I$ for some positive integer $n$. Note $(r^m)^n = r^{mn} \in I$, so $r \in \sqrt{I}$. Thus $\sqrt{\sqrt{I}} \subseteq \sqrt{I}$.

        Now suppose $r \in \sqrt{I}$, meaning that $r^n \in I$ for some positive integer $n$. Note that $r = r^1 \in \sqrt{I}$, so $r \in \sqrt{\sqrt{I}}$. Therefore we have $\sqrt{I} \subseteq \sqrt{\sqrt{I}}$.

        Therefore, since $\sqrt{\sqrt{I}} \subseteq \sqrt{I}$ and $\sqrt{I} \subseteq \sqrt{\sqrt{I}}$, we must have $\sqrt{\sqrt{I}} = \sqrt{I}$.
    ...
    \item \begin{partquestions}{\alph*}
        ...
        On the other hand, suppose $a \in d\Z$, meaning $a = dt$ for some integer $t$. By B\'{e}zout's lemma (\myref{lemma-bezout}), write $d = mx + ny$ for some integers $x$ and $y$. Hence
        \begin{align*}
            a &= (mx + ny)t\\
            &= m(xt) + n(yt)\\
            &\in m\Z + n\Z
