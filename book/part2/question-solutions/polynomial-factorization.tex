...
\begin{questions}
    ...
    \begin{table}[H]
        ...
        \end{tabular}
    \end{table}
    We therefore see that $f(x)$ has zeroes in $\Z_3$ and $\Z_5$ and no zeroes in $\Z_2$ and $\Z_7$. Thus $f(x)$ is irreducible over $\Z_2$ and $\Z_7$ and reducible in $\Z_3$ and $\Z_5$ by \myref{thrm-degree-2-or-3-irreducible-iff-has-no-zeroes}.
    ...
    \item Since $p(x)$ is irreducible we know $D[x]/\princ{p(x)}$ is a field (\myref{corollary-polynomial-quotient-by-principal-ideal-is-field-iff-polynomial-irreducible}), which is a integral domain (\myref{prop-field-is-integral-domain}). Thus we see that $\princ{p(x)}$ is a prime ideal (\myref{thrm-prime-ideal-iff-quotient-ring-is-integral-domain}). As $p(x) \vert a(x)b(x)$ therefore $a(x)b(x) \in \princ{p(x)}$. Hence $a(x) \in \princ{p(x)}$ or $b(x) \in \princ{p(x)}$ by definition of a prime ideal. So $a(x) = k(x)p(x)$ or $b(x) = k(x)p(x)$ for some $k(x) \in D[x]$, meaning $p(x) \vert a(x)$ or $p(x) \vert b(x)$.
...
\begin{questions}
    ...
    \item \begin{partquestions}{\alph*}
        ...
        \begin{partquestions}{\roman*}
            ...
            Also, as $p$ is an odd integer, there exists a primitive root modulo $p$ (\myref{axiom-primitive-root-modulo-p}). Therefore we see that $\Un{p}$ is cyclic (\myref{prop-Un-cyclic-only-if-exists-primitive-root}), which therefore means that $\Un{p}$ is a cyclic group with even order.
