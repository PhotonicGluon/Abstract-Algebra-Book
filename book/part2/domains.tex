...
\end{definition}
\begin{remark}
    Most texts prefer to define the norm without the absolute value. However, defining it with the absolute value makes some properties easier to explain and prove.
\end{remark}
...
\begin{proof}
    ...
    \]
    This is equivalent to showing that $(a_1d_1+b_1c_1, b_1d_1) \mathrel{\sim} (a_2d_2+b_2c_2, b_2d_2)$ by \myref{thrm-equivalence-class-equivalence}, i.e.
    \[
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item If there exists (not necessarily unique) irreducible elements $q_1, q_2, \dots, q_m \in D$ such that $x = q_1q_2q_3\cdots q_m$ where $m$ is a positive integer, then
        \begin{itemize}
            \item $m=n$; and
...
\begin{lemma}\label{lemma-reducible-in-field-of-fractions-means-reducible-in-UFD}
    Let $D$ be a UFD and let $\Frac{D}$ be its field of fractions. Let $f(x) \in D[x]$, and suppose $f(x) = p(x)q(x)$ where $p(x), q(x) \in \Frac{D}[x]$. Then there exist $P(x), Q(x) \in D[x]$ such that $f(x) = P(x)Q(x)$, $\deg P(x) = \deg p(x)$, and $\deg Q(x) = \deg q(x)$.
\end{lemma}
