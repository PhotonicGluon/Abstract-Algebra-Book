...
In an earlier chapter, we introduced domains before focusing mainly on integral domains. We then explored concepts of factorization but restricted the discussion to polynomial rings. We explore more types of domains here and the factorization available in such domains.
...
\begin{example}
    2 and -2 are associates in $\Z$ since $-2 = (-1)(2)$ and -1 is a unit in $\Z$.
\end{example}

\begin{example}
    3 and 5 are associates in $\Z_7$ since $5 = 12 = (4)(3)$. In fact, any two non-zero elements in $\Z_7$ are associates of each other, since every non-zero element of $\Z_7$ is a unit.
\end{example}
...
Thus, in a sense, it is only meaningful to talk about associates when we restrict our view to that of integral domains that are \textbf{not} fields.
...
We note that the definition of irreducibles is similar to that of irreducible polynomials, and the definition of primes is similarly defined to that of prime ideals. In fact, we have the following theorem linking prime elements to prime ideals.
...
Let $d$ be a square-free integer not equal to 1. We note that
\[
    \Z[\sqrt{d}] = \left\{a + b\sqrt{d} \vert a,b \in \Z\right\}
\]
forms an integral domain. We will leave proving that this is indeed an integral domain to the next section. We also define the norm, which is the map $N: \Z[\sqrt{d}] \to \Z$ where
\[
    N(a+b\sqrt{d}) = \left|a^2 - db^2\right|,
\]
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \item We claim that $r$ is not a prime. Observe that
        \[
            (1+\sqrt{-3})(1-\sqrt{-3}) = 1-(-3) = 4
        \]
        so $1 + \sqrt{-3}$ divides $4 = 2 \times 2$. However, for integers $a$ and $b$ to exist such that $2 = (1+\sqrt{-3})(a+b\sqrt{-3}) = (a-3b) + (a+b)\sqrt{-3}$, we must have $a - 3b = 2$ and $a + b = 0$, which has a solution $a = \frac12$ and $b = -\frac12$ which are not integers. Therefore $r$ is not prime.
    \end{itemize}
...
We alluded to quadratic integers in the previous section; we formally define what they are here.

\begin{definition}
    Let $d$ be a square-free integer not equal to 1. Then the \term{quadratic field}\index{quadratic field} is
    \[
...
\end{proposition}
\begin{proof}
    We note that the arithmetic in the quadratic field was not specified explicitly; this is because we are adapting the definitions of polynomial addition and multiplication to our current context. In particular,
    \[
    ...
    Note we do not need to prove that $(\Q[\sqrt{d}], +)$ is an abelian group as this is inherited from our adaptation of the polynomial ring $\Q[x]$, where we substitute $x$ with $\sqrt{d}$. We, however, need to prove that multiplication has identity and inverses, since the rest is also inherited from polynomial multiplication.

    \begin{itemize}
        \item \textbf{Identity}: The multiplicative identity is $1 = 1 + 0\sqrt{d}$, since one sees clearly that
        \begin{align*}
            &(a+b\sqrt{d})(1+0\sqrt{d})\\
            &= ((a\times1) + (b\times0)d) + ((a\times0) + (b \times 1))\sqrt{d}\\
            &= a + b\sqrt{d}
        \end{align*}
        ...
        \begin{align*}
            ...
            &= \left(a\left(\frac{a}{a^2-db^2}\right) + b\left(-\frac{b}{a^2-db^2}\right)d\right)\\
            &\quad\quad+ \left(a\left(-\frac{b}{a^2-db^2}\right) + b\left(\frac{a}{a^2-db^2}\right)\right)\sqrt{d}\\
...
\begin{definition}
    The \term{norm}\index{quadratic integer!norm} is the map $N: \Z[\sqrt{d}] \to \Z$ given by
    \[
        N(a+b\sqrt{d}) = \left|a^2-db^2\right|
    \]
    where $a + b\sqrt{d} \in \Z[\sqrt d]$ and $d$ is a square-free integer not equal to 1.
\end{definition}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            &N(xy)\\
            &= N\left((p+q\sqrt{d})(r+s\sqrt{d})\right)\\
        ...
        \item For the forward direction, if $x$ is a unit then there exists a $x^{-1} \in \Z[\sqrt d]$ such that $xx^{-1} = 1$. Note $N(xx^{-1}) = N(x)N(x^{-1}) = N(1) = 1$. Therefore, since we are working in the integers, we see $N(x)$ and $N(x^{-1})$ are both 1.
...
\end{definition}
\begin{proposition}
    Let $d$ be a square-free integer not equal to 1. Then $\Z[\sqrt{d}]$ is a subring of $\Q[\sqrt{d}]$. Furthermore $\Z[\sqrt{d}]$ is an integral domain.
\end{proposition}
\begin{proof}
    ...
    \[
        (a+b\sqrt{d})(x+y\sqrt{d}) = (ax+(by)d) + (ay+bx)\sqrt{d}
    \]
    which is also in $\Z[\sqrt{d}]$. Thus $\Z[\sqrt{d}]$ is closed under multiplication. So $\Z[\sqrt{d}]$ is a subring of $\Q[\sqrt{d}]$. Furthermore, since $\Q[\sqrt{d}]$ is a field, thus the multiplication is commutative, which means $\Z[\sqrt{d}]$ is a commutative subring.
...
\begin{exercise}
    ...
    \begin{partquestions}{\alph*}
        ...
        \item \begin{partquestions}{\roman*}
            ...
            \item Prove that 3 is a prime in the Gaussian integers.
        \end{partquestions}
    \end{partquestions}
\end{exercise}
\begin{remark}
    A prime in the Gaussian integers is known as a \term{Gaussian prime}\index{Gaussian prime}.
\end{remark}
...
\begin{definition}\label{definition-field-of-fractions}
    ...
    \]
    for all $(a,b),(c,d) \in D \times D^\ast$. Denote the equivalence class $[(a, b)]$ by $\frac ab$ for any $(a,b) \in D \times D^\ast$. Let $\Frac{D}$ be the set of equivalence classes of $D \times D^\ast$ under $\sim$. Then $\Frac{D}$ is called the \term{field of fractions}\index{field of fractions} of $D$ under addition and multiplication given by
    \begin{align*}
        \frac ab + \frac cd &= \frac{ad+bc}{bd} \text{ and}\\
        \frac ab \times \frac cd &= \frac{ac}{bd}
    \end{align*}
    respectively for all $(a,b),(c,d) \in D \times D^\ast$.
\end{definition}
\begin{remark}
...
\begin{proposition}
    The relation $\sim$ defined in in \myref{definition-field-of-fractions} is indeed an equivalence relation.
\end{proposition}
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \textbf{Symmetric}: Suppose now that $(a,b)\mathrel{\sim}(c,d)$ for some $(a,b), (c,d) \in D \times D^\ast$. Then $ad = bc$. Again, since $D$ is commutative, we see $cb = da$. Hence, using the definition of $\sim$, we see $(c,d)\mathrel{\sim}(a,b)$, so $\sim$ is symmetric.

        \item \textbf{Transitive}: Suppose $(a,b)\mathrel{\sim}(c,d)$ and $(c,d)\mathrel{\sim}(e,f)$ for some $(a,b), (c,d), (e,f) \in D \times D^\ast$. Thus $ad = bc$ and $cf = de$. Multiplying both sides of $ad = bc$ by $f$, one sees
        \begin{align*}
            (af)d &= (ad)f\\
...
With the equivalence relation verified, we can now check that the operations defined in $\Frac{D}$ are, indeed, well-defined operations. Remember that we define the equivalence class of $[(a, b)]$ to be $\frac ab$.

\begin{proposition}
    Addition and multiplication as defined in \myref{definition-field-of-fractions} are well-defined operations.
\end{proposition}
\begin{proof}
    We first prove that addition is well-defined. Suppose $\frac{a_1}{b_1} = \frac{a_2}{b_2}$ and $\frac{c_1}{d_1} = \frac{c_2}{d_2}$ for some $(a_1,b_1), (a_2,b_2), (c_1,d_1), (c_2,d_2) \in D \times D^\ast$. We show that $\frac{a_1}{b_1} + \frac{c_1}{d_1} = \frac{a_2}{b_2} + \frac{c_2}{d_2}$, i.e.
    \[
        \frac{a_1d_1+b_1c_1}{b_1d_1} = \frac{a_2d_2 + b_2c_2}{b_2c_2}.
    \]
    This is equivalent to showing $(a_1d_1+b_1c_1, b_1d_1) \mathrel{\sim} (a_2d_2+b_2c_2, b_2d_2)$ by \myref{thrm-equivalence-class-equivalence}, i.e.
    \[
        (a_1d_1+b_1c_1)(b_2d_2) = (b_1d_1)(a_2d_2+b_2c_2).
    \]
    As $\frac{a_1}{b_1} = \frac{a_2}{b_2}$, we know that $(a_1, b_1) \mathrel{\sim} (a_2, b_2)$ (\myref{thrm-equivalence-class-equivalence}) which means $a_1b_2 = b_1a_2$. Similarly, as $\frac{c_1}{d_1} = \frac{c_2}{d_2}$ thus $c_1d_2 = d_1c_2$. Hence we see
    \begin{align*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{Associativity}: Let $\frac ab, \frac cd, \frac ef \in \Frac{D}$. Then
            \begin{align*}
                &\frac ab \times \left(\frac cd \times \frac ef\right)\\
                &= \frac ab \times \frac {ce}{df}\\
                &= \frac {a(ce)}{b(df)}\\
                &= \frac {(ac)e}{(bd)f}\\
        ...
        \item \textbf{Distributive}: Let $\frac ab, \frac cd, \frac ef \in \Frac{D}$. We prove only $\frac ab \times \left(\frac cd + \frac ef\right) = \frac ab \times \frac cd + \frac ab \times \frac ef$ since the other case would be proven after we have shown that multiplication is commutative.
...
To simplify the notation used when working with the field of fractions, $\Frac{D}$, of an integral domain $D$, we write $x$ in place of the equivalence class $\frac x1$ for any element $x \in D$.
...
\begin{example}
    ...
    \begin{itemize}
        \item \textbf{Well-defined}: Suppose $[(a, b)], [(c, d)] \in \Frac{\Z}$ where $[(a,b)]=[(c,d)]$. Then $(a, b) \mathrel{\sim} (c, d)$ by \myref{thrm-equivalence-class-equivalence}, i.e. $ad = bc$. Hence one sees clearly that $\frac ab = \frac cd$. Therefore,
        \[
...
\end{example}
\begin{remark}
    When working with functions where the function domain is the field of fractions, one must take care to prove that the map is well-defined.
\end{remark}
...
\begin{example}\label{example-field-of-fractions-of-gaussian-integers-isomorphic-to-quadratic-integers-of-i}
    ...
    Define the map $\phi: \Frac{\Z[i]} \to \Q[i]$ where $\phi([(w, z)]) = \frac wz$. One can see that $\frac wz$ is indeed in $\Q[i]$, since if $w = a+bi\in\Z[i]$ and $z = c+di \in \Z[i]$ with $z \neq 0$ then
    \begin{align*}
...
\begin{exercise}
    ...
    \begin{partquestions}{\alph*}
        ...
        \item $\Frac{k\Z} \cong \Q$ for all non-zero $k \in \Z$.
    \end{partquestions}
\end{exercise}

\section{Unique Factorization Domains (UFDs)}
We now have the necessary terminology to formalize the idea of a unique factorization domain.

\begin{definition}
    An integral domain $D$ is a \term{unique factorization domain (UFD)}\index{unique factorization domain}\index{UFD} if and only if the following conditions are met.
    \begin{enumerate}
        \item For every non-zero non-unit $x \in D$, there exists (not necessarily unique) irreducible elements $p_1, p_2, \dots, p_n \in D$ such that $x = p_1p_2p_3\cdots p_n$ where $n$ is a positive integer.
        \item If there are (not necessarily unique) irreducible $q_1, q_2, \dots, q_m \in D$ such that $x = q_1q_2q_3\cdots q_m$ where $m$ is a positive integer, then
        \begin{itemize}
            \item $m=n$; and
            \item there exists a bijection $\sigma: \{1,2,\dots,n\} \to \{1,2,\dots,m\}$ such that $p_i$ and $q_{\sigma(i)}$ are associates for all $i \in \{1,2,\dots,n\}$.
        \end{itemize}
    \end{enumerate}
\end{definition}

Loosely, in a unique factorization domain,
\begin{enumerate}
    \item every non-zero non-unit element can be written as a product of irreducibles; and
...
\begin{definition}
    If every strictly increasing sequence of principal ideals
    \[
        I_1 \subset I_2 \subset I_3 \subset \cdots
    \]
    is finite in length, then the \term{ascending chain condition on principal ideals (ACCP)} of a ring $R$\index{ascending chain condition on principal ideals of a ring}\index{ACCP} is satisfied.
\end{definition}
...
\end{theorem}
\begin{proofsketch}
    We need to show two parts. The first is to show existence; the second is to show uniqueness.
...
\begin{proof}[Proof (see {\cite[Theorem 18.3]{gallian_2016}})]
    ...
    We now show the uniqueness of such a factorization, up to associates and the reordering of terms. We first suppose that some non-zero non-unit element of $D$, say $a$, can be written as
    \[
    ...
    \]
    by cancellation law (\myref{prop-domain-cancellation-law}). Since any associate of an irreducible is also irreducible (\myref{prop-associates-of-irreducible-is-irreducible}), we might as well write $uq_2$ as $Q$, which is an irreducible, meaning
    \[
    ...
    Hence the factorization is unique by induction.
\end{proof}
...
\begin{proof}[Proof (see {\cite[Theorem 18.24]{judson_beezer_2022}})]
    ...
    \]
    Since $p$ divides all of $a_1, a_2, \dots, a_{r-1}$ and $p$ divides all of $b_1, b_2, \dots, b_{s-1}$, we see that $p$ divides every term except for $a_rb_s$. However, since $p$ divides all coefficients of $f(x)g(x)$, we know $p$ divides $c_{r+s}$, which means that either $p \vert a_r$ or $p \vert b_s$, a contradiction.

    Therefore no prime $p$ divides the coefficients of $f(x)g(x)$, meaning  the GCD of $f(x)g(x)$ is 1. Hence $f(x)g(x)$ is primitive.
\end{proof}
...
\begin{lemma}\label{lemma-reducible-in-field-of-fractions-means-reducible-in-UFD}
    Let $D$ be a UFD and let $\Frac{D}$ be its field of fractions. Let $f(x) \in D[x]$, and suppose $f(x) = p(x)q(x)$ where $p(x), q(x) \in \Frac{D}[x]$. Then there exist polynomials $P(x), Q(x) \in D[x]$ such that $f(x) = P(x)Q(x)$, $\deg P(x) = \deg p(x)$, and $\deg Q(x) = \deg q(x)$.
\end{lemma}
\begin{proof}[Proof (see {\cite[Lemma 18.26]{judson_beezer_2022}})]
    Let $f(x) \in D[x]$, and suppose $f(x) = p(x)q(x)$ where $p(x), q(x) \in \Frac{D}[x]$. Choose $a, b\in D$ such that $ap(x), bq(x) \in D[x]$. So we can find $C_P, C_Q \in D$ such that $ap(x) = C_P P(x)$ and $bq(x) = C_Q Q(x)$ where $P(x)$ and $Q(x)$ are primitive polynomials in $D[x]$. Therefore
    \[
...
We can finally prove the result that we have been alluding to.

\begin{corollary}\label{corollary-Z-is-UFD}
    $\Z[x]$ is a UFD.
\end{corollary}
\begin{proof}
    Since $\Z$ is a PID (\myref{prop-Z-is-PID}), therefore $\Z$ is a UFD by \myref{thrm-PID-is-UFD} which means that $\Z[x]$ is a UFD (\myref{thrm-UFD-iff-polynomial-ring-is-UFD}).
\end{proof}
...
We repeatedly used Euclid's division lemma (\myref{lemma-euclid-division}) to achieve division with remainder. We also seen polynomial long division (\myref{thrm-polynomial-long-division}) which is also similar to the division lemma. We generalize these properties abstractly, into what is known as a Euclidean domain.

\begin{definition}
    An integral domain $D$ is called a \term{Euclidean domain}\index{Euclidean domain} if and only if there is a map $N: D \setminus \{0\} \to \mathbb{N} \cup \{0\}$, called the \term{norm}\index{Euclidean domain!norm} or \term{measure}\index{Euclidean domain!measure}, that satisfies the following.
    \begin{enumerate}[leftmargin=3em,label=\textbf{EF\arabic*}.]
        \item For all non-zero $a, b \in D$ we have $N(a) \leq N(ab)$.
        \item If $n, d \in D$ and $d \neq 0$, then there exist elements $q$ and $r$ in $D$ such that $n = dq + r$, where $r = 0$ or $N(r) < N(d)$.
    \end{enumerate}
\end{definition}

\begin{example}
    The ring of integers, $\Z$, is a Euclidean domain with the norm $N(a) = |a|$, the absolute value of $a$.
    \begin{itemize}
        \item One sees that
        \[
            |a| \leq |a||b| = |ab|
        \]
        for all non-zero $a,b\in \Z$, so \textbf{EF1} is satisfied.
...
\begin{example}
    ...
    \begin{itemize}
        \item One sees that for all non-zero $f(x), g(x) \in F[x]$ we have
        \[
            \deg f(x) \leq \deg f(x) + \deg g(x) = \deg(f(x)g(x))
        \]
        by \myref{thrm-polynomial-degree-properties}, so \textbf{EF1} is satisfied.

        \item \textbf{EF2} is satisfied by polynomial long division (\myref{thrm-polynomial-long-division}).
    \end{itemize}
...
\begin{example}\label{example-gaussian-integers-is-euclidean-domain}
    ...
    \begin{itemize}
        \item Since $N(xy) = N(x)N(y)$ for all $x, y \in \Z[i]$ (\myref{prop-properties-of-quadratic-integer-norm}), therefore $N(x) \leq N(x)N(y) = N(xy)$ for all non-zero $x, y \in \Z[i]$, satisfying \textbf{EF1}.
...
\begin{figure}[h]
    \centering
    \pdfteximgframed[10.5pt]{0.65\textwidth}{part2/images/domains/class-inclusion.pdf_tex}
...
\begin{exercise}\label{exercise-domain-class-inclusion}
    Cite the appropriate theorems to explain the rest of the class inclusions in \myref{figure-domain-class-inclusion}.
\end{exercise}
...
\begin{problem}\label{problem-prime-element-divides-some-element-in-product}
    Let $D$ be an integral domain and let $p \in D$ be a prime. Suppose the elements $a_1, a_2, a_3, \dots, a_n \in D$ are such that $p \vert a_1a_2a_3\dots a_n$, where $n \geq 2$. Prove that $p \vert a_i$ for some $i \in \{1, 2, \dots, n\}$.
\end{problem}
...
\begin{problem}
    Suppose $D$ is an integral domain and $\phi: D \to \mathbb{N} \cup \{0\}$ is a non-constant function such that $\phi(xy) = \phi(x)\phi(y)$ for all $x,y\in D$. Prove that if $u \in D$ is a unit then $\phi(u) = 1$.
\end{problem}

\begin{problem}
    Prove that, for any field $F$, there exists infinitely many irreducible polynomials in $F[x]$ that are not associates.
\end{problem}
...
\begin{problem}[Sicherman Dice]\index{Sicherman Dice}
    ...
    \begin{partquestions}{\roman*}
        \item Explain why the coefficient of the term of degree $k$ (where $2 \leq k \leq 12$) in $P(x)$ gives exactly the number of ways that a pair of two dice could sum to $k$.
        ...
        \end{align*}
        where each $a_i$ and $b_i$ are positive integers. Suppose we have $P(x) = f(x)g(x)$. Based on \textbf{(ii)}, explain why
        \[
