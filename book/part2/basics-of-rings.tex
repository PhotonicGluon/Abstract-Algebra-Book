...
\begin{definition}
    Let the integer $n > 2$. The \term{ring of integers modulo $n$}\index{ring!of integers!modulo $n$} is $(\Z_n, \oplus_n, \otimes_n)$, where $\oplus_n$ and $\otimes_n$ denote addition and multiplication modulo $n$ respectively.
\end{definition}
\begin{remark}
...
\begin{definition}
    ...
    \]
    where $i = \sqrt{-1}$ is known as the \term{imaginary unit}\index{imaginary unit}, where $i^2 = -1$. Define complex addition and multiplication by
    \begin{align*}
        (a+bi) + (c+di) &= (a+c) + (b+d)i,\\
        (a+bi) \cdot (c+di) &= (ac-bd) + (ad+bc)i.
    \end{align*}
    Then $\C$ under complex addition and multiplication is the \term{ring of complex numbers}\index{ring!of complex numbers}.
\end{definition}
\begin{remark}
...
\begin{exercise}\label{exercise-ring-of-integers-is-a-ring}
    Prove that $\Z$ is a ring under regular addition and multiplication. You do \textit{not} need to prove the \textbf{Distributive} axiom.
\end{exercise}
...
We looked at matrices in the context of the General/Special Linear Group of matrices (\myref{section-groups-of-matrices}). Here we see that matrices in fact form rings, known as matrix rings. Before that though, we need to define the operations within that ring.
...
\section{More Definitions}\label{section-rings-more-definitions}
\begin{definition}
    Let $R$ be a ring. We say that a non-zero element $a \in R$ is a \term{zero divisor}\index{zero divisor} if there exists a non-zero element $b \in R$ such that $ab = 0$.
\end{definition}
\begin{example}
...
\begin{definition}
    Suppose $R$ is a ring with identity such that $0 \neq 1$. An element $u \in R$ is called a \term{unit}\index{unit} if there exists a $v \in R$ such that $uv=vu=1$.
...
\begin{definition}
    Suppose $R$ is a ring with identity such that $0 \neq 1$. If every non-zero element $x \in R$ is a unit, then $R$ is said to be a \term{division ring}\index{division ring}.
\end{definition}
...
\begin{problem}
    Let $R$ be a set with an operation $\ast$ such that for all $x, y \in R$ we have $x \ast y \in R$. If $(R, \ast, \ast)$ is a ring, describe the elements in $R$.
\end{problem}
