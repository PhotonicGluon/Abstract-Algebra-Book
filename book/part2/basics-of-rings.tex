...
\begin{definition}
    We say that $a \neq 0$ is a \term{zero divisor}\index{zero divisor} in a ring $R$ if there exists $b \neq 0$ such that $ab = 0$.
\end{definition}
\begin{example}
...
\begin{definition}
    Suppose $R$ is a ring with identity such that $0 \neq 1$. An element $u \in R$ is called a \term{unit}\index{unit} if there exists a $v \in R$ such that $uv=vu=1$. Equivalently, $u$ is a unit if it has a multiplicative inverse.
\end{definition}
\begin{example}
