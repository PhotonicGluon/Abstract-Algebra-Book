...
\begin{definition}
    Let $(R_1, +, \cdot)$ and $(R_2, \oplus, \otimes)$ be rings. A function $\phi: R_1 \to R_2$ is a \term{ring homomorphism}\index{homomorphism!ring} if and only if for all $a, b \in R_1$,
    \begin{align*}
...
Similar to groups, we write $R_1 \cong R_2$ if and only if $R_1$ and $R_2$ are ring isomorphisms.
...
\begin{exercise}\label{exercise-ring-image-of-identity-is-identity}
    ...
    \begin{partquestions}{\alph*}
        \item Show that $\phi(0_1) = 0_2$, where $0_1$ and $0_2$ is the additive identity of $R_1$ and $R_2$ respectively.
        \item If $R_1$ and $R_2$ are division rings, then show that $\phi(1_1) = 1_2$, where $1_1$ and $1_2$ is the multiplicative identity of $R_1$ and $R_2$ respectively.
    \end{partquestions}
\end{exercise}
...
\begin{exercise}
    ...
    \]
    and by using the FRIT (\myref{thrm-ring-isomorphism-1}).
\end{exercise}
...
\begin{example}\label{example-homomorphisms-from-Z12-to-Z28}
    ...
    By \myref{prop-homomorphism-on-multiplicative-identity-is-idempotent}, $\phi(1)$ is an idempotent. However, we cannot just assume that 0 and 1 are the \textit{only} idempotents in $\Z_{28}$; we need to check for them exhaustively.
    ...
    \begin{itemize}
        ...
        \item $|1|_+ = 28$ which does not divide 12, so 1 is not a valid value of $\phi(1)$. This is one thing that differed from the previous example, where 1 was a possible value of $\phi(1)$.
...
\begin{problem}\label{problem-ring-isomorphism-2}
    Let $R$ be a ring with a subring $S$ and an ideal $I$.
    \begin{partquestions}{\roman*}
