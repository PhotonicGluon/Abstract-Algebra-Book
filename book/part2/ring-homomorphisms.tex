...
\begin{example}\label{example-homomorphisms-from-Z12-to-Z28}
    ...
    By \myref{prop-homomorphism-on-multiplicative-identity-is-idempotent}, $\phi(1)$ is an idempotent, so we need to find all idempotents of $\Z_{28}$. However, we cannot just assume that 0 and 1 are the \textit{only} idempotents in $\Z_{28}$; we need to check for them exhaustively.
