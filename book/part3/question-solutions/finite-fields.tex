...
\begin{questions}
    ...
    \item In \myref{thrm-finite-field-is-perfect} we showed that the Frobenius endomorphism $\phi: F \to F, x \mapsto x^p$ is actually an automorphism. The result yields immediately since an automorphism is an isomorphism, meaning that every $a \in F$ has a unique $b \in F$ such that $a = b^p$.
    ...
    \item \begin{partquestions}{\roman*}
        ...
        \begin{align*}
            ...
            &= 1
        \end{align*}
        which means that $|\alpha| \leq 13$. Thus $\alpha$ is not a generator of $F^\ast$.

        \item We claim that $2\alpha$ is a generator of $F^\ast$. Since the order of an element must divide the order of a group (\myref{corollary-order-of-group-multiple-of-order-of-element}) we thus know that the order of $2\alpha$ must be 1, 2, 13, or 26.

        Clearly $2\alpha \neq 1$ and so $|2\alpha| \neq 1$. Also we clearly see $(2\alpha)^2 = 4\alpha^2 = \alpha^2 \neq 1$ so $|2\alpha| \neq 2$. Finally, we see
        \begin{align*}
            (2\alpha)^{13} &= 2^{13}\alpha^{13}\\
            &= 2^{13} & (\text{since } \alpha^{13} = 1 \text{ from \textbf{(i)}})\\
            &= 8192\\
            &= 2\\
            &\neq 1
        \end{align*}
    ...
    \]
    by Tower Law (\myref{thrm-tower-law}), meaning $[\GF{p^{mn}}:\GF{p}(\alpha)] = 1$. Hence $\GF{p^{mn}} = \GF{p}(\alpha)$ by \myref{prop-finite-extension-of-degree-1-means-extension-equals-base-field}.
