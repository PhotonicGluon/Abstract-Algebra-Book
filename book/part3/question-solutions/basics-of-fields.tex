...
\begin{questions}
    \item \begin{partquestions}{\alph*}
        \item Clearly $0 = 0 - 0$ since $x - x = 0$ for all elements $x \in F$, including for $x = 0$. But since 0 is the additive identity, this means $0 + x = x$ for all $x \in F$, including $x = -0$. Hence $0 = -0$.
        \item Clearly $1 = 1\times 1^{-1}$ since $xx^{-1} = 1$ for all $x \in F^\ast$, including for $x = 1$. But since 1 is the multiplicative identity, this means $1x = x$ for all $x \in F$, including $x = 1^{-1}$. So $1 = 1^{-1}$.
    \end{partquestions}
...
\begin{questions}
    ...
    \item Disprove. Suppose $\phi: \C \to \R$ is an isomorphism. Then there must exist $w \in \C$ such that $\phi(w) = -1$. Note that $\sqrt{w} \in \C$; let $z = \sqrt{w}$. Then
    \[
    ...
    \item \begin{partquestions}{\roman*}
        ...
        \item Define the map $\phi: \Z_5 \to R$ where $r \mapsto r + \princ{2-i}$. We show that $\phi$ is an isomorphism.
        \begin{itemize}
        ...
        \end{itemize}
        Therefore we see $\phi$ is an isomorphism, which means that $R \cong \Z_5$, thus showing that $R$ is a field of order 5.
    \end{partquestions}
    ...
    \item Note that $\ker\phi$ is an ideal of $F$. However, any field has no proper ideals (\myref{prop-ring-is-field-iff-no-proper-ideals}). So $\ker\phi = F$ or $\ker\phi = \{0\}$.
    ...
    \item \begin{partquestions}{\roman*}
        ...
        \item Since $q$ is another prime dividing $|F|$, it is not $p$. Thus $p$ and $q$ are coprime, meaning that there exist integers $\lambda$ and $\mu$ such that $\lambda p + \mu q = 1$ by B\'ezout's lemma (\myref{lemma-bezout}). Hence $(\lambda p + \mu q)x = x$ for any element $x \in F$.
        ...
        Therefore no other prime other than $p$ divides $|F|$, meaning that $|F| = p^n$ where $n$ is a \textbf{non-negative} integer. But $n \neq 0$ (since otherwise $F$ has order 1) which means that the multiplicative group has order 0, an impossibility. Thus $|F| = p^n$ where $n$ is a \textbf{positive} integer.
    \end{partquestions}

    \item \begin{partquestions}{\alph*}
        \item We show that there are infinitely many equivalence classes of the form $\frac{f(x)}1$.
        ...
        \begin{align*}
            &\frac{x^nf_1(x)f_2(x)\cdots f_n(x)}{1}\\
            &= \left(\frac{xf_1(x)}{1}\right)\left(\frac{xf_2(x)}{1}\right)\cdots\left(\frac{xf_n(x)}{1}\right).
        \end{align*}
        ...
        \item Let $\frac{f(x)}{g(x)} \in \Frac{\Z_p[x]}$. Write $f(x) = a_0 + a_1x + a_2x^2 + \cdots + a_nx^n$ where $a_i \in \Z_p$. Note that $pa = 0$ for all $a \in \Z_p$ since $\Char{\Z_p} = p$. So we see
        \begin{align*}
            &p\left(\frac{f(x)}{g(x)}\right)\\
            &= \underbrace{\frac{f(x)}{g(x)} + \frac{f(x)}{g(x)} + \cdots + \frac{f(x)}{g(x)}}_{p \text{ times}}\\
            &= \frac{pf(x)}{g(x)}\\
            &= \frac{(pa_0) + (pa_1)x + (pa_2)x^2 + \cdots + (pa_n)x^n}{g(x)}\\
            &= \frac{0 + 0x + 0x^2 + \cdots + 0x^n}{g(x)}\\
            &= \frac0{g(x)}\\
            &= 0.
        \end{align*}
