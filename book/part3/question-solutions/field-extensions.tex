...
\begin{questions}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            &= \phi\left(((ad + bc)x + (bd - ac)) + \princ{x^2+1}\right) & (x^2 = -1)\\
            &= (bd - ac) + (ad + bc)i\\
            &= (b + ai)(d + ci)\\
            &= \phi\left(ax + b + \princ{x^2+1}\right)\phi\left(cx + d + \princ{x^2+1}\right)
        \end{align*}
    ...
    \item For brevity let $K = F(a_1, a_2, \dots, a_{n-1})(a_n)$. Note that $K$ contains the extension $F(a_1, a_2, \dots, a_{n-1})$ and the element $a_n$, and that the extension $F(a_1, a_2, \dots, a_{n-1})$ contains the field $F$ and the elements $a_1, a_2, \dots, a_{n-1}$. Thus $K$ contains $F$ as well as $a_1, a_2, \dots, a_{n-1}$, and $a_n$, which means that $K$ is included in the intersection that generates $F(a_1, a_2, \dots, a_n)$, so $K \subseteq F(a_1, a_2, \dots, a_n)$. But $F(a_1, a_2, \dots, a_n)$ is the smallest subfield of $E$ that contains $F$ and the elements $a_1, a_2, \dots, a_n$. Therefore $K = F(a_1, a_2, \dots, a_n)$ as required.
    ...
    When $n = 1$, one clearly sees that $((f(x))^1)' = (f(x))' = f'(x) = 1(f(x))^{1-1}f'(x)$.
    ...
    Therefore we have $((f(x))^n)' = n(f(x))^{n-1}f'(x)$ for all positive integers $n$ by mathematical induction.
    ...
    \begin{align*}
        ...
        &= (x^{p^k}+y^{p^k})^p & (\text{induction hypothesis})\\
        &= \left(x^{p^k}\right)^p + \left(y^{p^k}\right)^p & (\myref{prop-freshman-dream})\\
        &= x^{p^{k+1}} + y^{p^{k+1}}
    \end{align*}
...
\begin{questions}
    ...
    \item \begin{partquestions}{\alph*}
        ...
        {
            \fontsize{8.75pt}{11pt}\selectfont
            \begin{align*}
    ...
    \begin{partquestions}{\roman*}
        \item Since $p(x)$ is irreducible, thus we see that $\princ{p(x)}$ is a maximal ideal (\myref{thrm-irreducible-iff-principal-ideal-is-maximal}). Therefore $\Z_2[x]/\princ{p(x)}$ is a field by \myref{thrm-maximal-ideal-iff-quotient-ring-is-field}.
    ...
    Now, seeking a contradiction, suppose $f(x)$ is reducible over $F$, which means that there exist non-constant $g(x), h(x) \in F[x]$ such that $f(x) = g(x)h(x)$. In particular, we see $g(x) = (x-\beta)^n$ for some positive integer $n$ where $1 \leq n \leq p$. Expanding $g(x)$ using the Binomial Theorem (\myref{thrm-binomial}) yields
    \[
