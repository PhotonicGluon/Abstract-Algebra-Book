...
\begin{definition}
    Let $F$ be a field and $E/F$ be a field extension. Then $\alpha \in E$ is \term{algebraic}\index{algebraic}\index{algebraic!element} over $F$ if and only if there exists some non-constant polynomial $f(x) \in F[x]$ such that $\alpha$ is a zero of $f(x)$.

    A non-algebraic element over $F$ is called \term{transcendental}\index{transcendental} over $F$.
\end{definition}
...
\begin{example}
    Both $\pi$ and $e$ are algebraic over $\R$ since they are the zeroes of the polynomials $x - \pi$ and $x - e$ respectively. However, it is not a trivial task to show that $\pi$ and $e$ are not algebraic (i.e., transcendental) over $\Q$.
\end{example}
...
\begin{proof}
    ...
    \begin{itemize}
        \item \textbf{Well-Defined}: Suppose $\frac{f(x)}{g(x)}, \frac{p(x)}{q(x)} \in \Frac{F[x]}$ such that $\frac{f(x)}{g(x)} = \frac{p(x)}{q(x)}$. Thus $f(x)q(x) = p(x)g(x)$ by definition of equality of equivalence classes in the field of fractions of $F[x]$. Thus we see $f(\alpha)q(\alpha) = p(\alpha)g(\alpha)$ which quickly means $f(\alpha)(g(\alpha))^{-1} = p(\alpha)(q(\alpha))^{-1}$. So
        \[
        ...
        \begin{align*}
            &\psi\left(\frac{f(x)}{g(x)} + \frac{p(x)}{q(x)}\right)\\
            &= \psi\left(\frac{f(x)q(x) + g(x)p(x)}{g(x)q(x)}\right)\\
...
For an algebraic element $\alpha$, \myref{thrm-characterisation-of-extensions} gives us an irreducible polynomial $p(x)$ of minimum degree which has a zero of $\alpha$. We have a special name for $p(x)$ if it is monic.
...
There are a few special properties of the minimal polynomial that we will introduce here.
...
\end{corollary}
\begin{proof}
    Consider the map $\phi: F[x] \to F(\alpha)$ where $f(x) \mapsto f(\alpha)$. From \myref{thrm-characterisation-of-extensions} we know that $\ker\phi$ is non-empty since $\alpha$ is algebraic over $F$. For brevity, let $J_\alpha = \ker\phi$.
...
\end{corollary}
\begin{proof}
    Let $p(x), f(x) \in F[x]$ be as above. Using polynomial long division (\myref{thrm-polynomial-long-division}) with divisor $p(x)$ we see
    \[
        f(x) = q(x)p(x) + r(x)
    \]
...
\begin{proof}[Proof (see {\cite[Theorem 21.5]{gallian_2016}})]
    Let $[K:E] = m$ and $[E:F] = n$. Also let $X = \{\mathbf{x}_1, \mathbf{x}_2, \dots, \mathbf{x}_m\}$ be a basis for $K$ over $E$ and $Y = \{\mathbf{y}_1, \mathbf{y}_2, \dots, \mathbf{y}_n\}$ be a basis for $E$ over $F$. It suffices to prove that
    \[
        YX = \{\mathbf{y}_j\mathbf{x}_i \vert 1 \leq i \leq m \text{ and } 1 \leq j \leq n\}
    \]
    ...
    \[
        r = a_1\mathbf{x}_1 + a_2\mathbf{x}_2 + \cdots + a_m\mathbf{x}_m.
    \]
    Furthermore, for each $a_i$, there are elements $b_{i,1}, b_{i,2}, \dots, b_{i,n}\in F$ such that
    \[
        a_i = b_{i,1}\mathbf{y}_1 + b_{i,2}\mathbf{y}_2 + \cdots + b_{i,n}\mathbf{y}_n.
    \]
    So we see
    \[
        r = \sum_{i=1}^m\left(\left(\sum_{j=1}^n b_{i,j}\mathbf{y}_j\right)\mathbf{x}_i\right) = \sum_{i=1}^m\sum_{j=1}^nb_{i,j}(\mathbf{y}_j\mathbf{x}_i)
    \]
    ...
    \[
        \sum_{i=1}^m\left(\left(\sum_{j=1}^n b_{i,j}\mathbf{y}_j\right)\mathbf{x}_i\right) = \sum_{i=1}^m\sum_{j=1}^nb_{i,j}(\mathbf{y}_j\mathbf{x}_i) = 0.
    \]
    Recall from above that
    \[
         \sum_{i=1}^m\sum_{j=1}^nb_{i,j}(\mathbf{y}_j\mathbf{x}_i) = \sum_{i=1}^m\left(\left(\sum_{j=1}^n b_{i,j}\mathbf{y}_j\right)\mathbf{x}_i\right)
    \]
    so we see
    \[
        \sum_{i=1}^m\left(\left(\sum_{j=1}^n b_{i,j}\mathbf{y}_j\right)\mathbf{x}_i\right) = 0.
    \]
    As $\displaystyle \sum_{j=1}^n b_{i,j}\mathbf{y}_j \in E$ and $X$ is a basis for $K$ over $E$, we must have $\displaystyle \sum_{j=1}^n b_{i,j}\mathbf{y}_j = 0$ for each $i$. But each $b_{i,j} \in F$ and $Y$ is a basis for $E$ over $F$, so $b_{i, j} = 0$. Therefore $YX$ is linearly independent over $F$.
...
\begin{figure}[H]
    \centering
    \pdfteximg{0.25\textwidth}{part3/images/algebraic-extensions/multiplying-degrees.pdf_tex}
...
\begin{examplewithcutout}{right}{0}{0.6\textwidth}{0pt}{8}{
    \begin{figure}[H]
        \centering
        \pdfteximg[9pt]{0.625\linewidth}{part3/images/algebraic-extensions/Q-cbrt2-4root3.pdf_tex}
    \end{figure}
}
...
\begin{exercise}
    Find $[\Q(\sqrt2,\sqrt[3]3):\Q]$. Hence find $[\Q(\sqrt2, \sqrt[3]3, \sqrt[6]{72}):\Q]$.
\end{exercise}

\begin{exercise}
    Suppose $\alpha$ is an element of some extension of $\Q$ with a minimal polynomial of $p(x) = x^5 + 2x + 1 \in \Q[x]$. Prove that $\sqrt2 \notin \Q(\alpha)$.
\end{exercise}

\begin{example}
    Recall from \myref{exercise-splitting-field-sqrt2-sqrt3} that $\Q(\sqrt2, \sqrt3) = \Q(\sqrt2 + \sqrt3)$. We present another method for proving this fact.
    ...
    \[
        \frac12\left(\sqrt2 + \sqrt3\right) + \frac12\left(\sqrt2 - \sqrt3\right) = \sqrt2
    \]
    and
    \[
        \frac12\left(\sqrt2 + \sqrt3\right) - \frac12\left(\sqrt2 - \sqrt3\right) = \sqrt3
    \]
...
\begin{proof}
    ...
    \end{align*}
    In particular we see $p(x) \in F_n[x]$. Therefore $[F_n(\alpha): F_n] = n$, which is finite. Also, since each $a_i$ is algebraic over $F$, thus each $[F_{i+1}:F_i] = [F_i(a_{i+1}):F_i]$ is finite. Therefore, by Tower Law (\myref{thrm-tower-law}), we see that
    \[
        [F_n(\alpha):F] = [F_n(\alpha):F_n][F_n:F_{n-1}][F_{n-1}:F_{n-2}]\cdots[F_1:F_0][F_0:F]
    \]
    is finite. Hence $F_n(\alpha)$ is a finite extension of $F$, meaning that $F_n(\alpha)$ is an algebraic extension of $F$ (\myref{thrm-finite-extension-is-algebraic}). So every element of $F_n(\alpha)$, including $\alpha$, is algebraic over $F$. Since $\alpha$ was arbitrary, therefore every element of $K$ is algebraic over $F$, which means that $K$ is an algebraic extension of $F$.
\end{proof}
...
\end{corollary}
\begin{proof}
    From \myref{thrm-tower-law}, we are able to construct a `tower' of extension fields of arbitrarily large degree. In particular, the element $\alpha$ chosen in that theorem was arbitrary but algebraic, and we may find any irreducible degree $n$ polynomial that we wish in order to create a degree $n$ element.
\end{proof}
...
\begin{definition}
    A field $F$ is \term{algebraically closed}\index{algebraically closed} if every non-constant polynomial in $F[x]$ has a zero in $F$.
\end{definition}
