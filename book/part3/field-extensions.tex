...
\end{proposition}
\begin{proof}
    We know that a field homomorphism is trivial or injective by \myref{thrm-homomorphism-from-field-is-injective-or-trivial}. Since $\phi$ is non-trivial, therefore $\phi$ is injective. Thus we see $F \cong \im\phi$. As $\im\phi$ is a subfield of $E$, therefore $E$ contains a subfield isomorphic to $F$.
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
        \overline{p}(x) &= (a_0 + \princ{p(x)}) + (a_1 + \princ{p(x)})x + (a_2 + \princ{p(x)})x^2 + \cdots\\
...
\begin{definition}
    ...
    \[
        F(a_1, a_2, \dots, a_n)
    \]
    and say that \term{$F(S)$ is finitely generated over $F$}\index{finitely generated}.
\end{definition}
...
\begin{proof}
    ...
    Now, if $f(x) + \princ{p(x)} \in F[x]/\princ{p(x)}$ then we may use polynomial long division (\myref{thrm-polynomial-long-division}) to write $f(x) = q(x)p(x) + r(x)$ where $r(x)$ is a unique polynomial where $r(x) = 0$ or $\deg r(x) < \deg p(x) = n$. Hence if $r(x) = a_0 + a_1x + \cdots + a_{n-1}x^{n-1}$ we see
    \[
...
\end{corollary}
\begin{proof}
    Note that $F[\alpha] \subseteq E$ contains both the entire ground field $F$ and the element $\alpha$. Since $F(\alpha)$ is the intersection of subfields that contains both $F$ and $\alpha$, thus $F(\alpha) \subseteq F[\alpha]$. But \myref{thrm-simple-extension-isomorphism} proves that $F(\alpha) \cong F[\alpha]$. Thus $F(\alpha) = F[\alpha]$.
\end{proof}
...
\begin{example}
    Let $f(x) = x^6 - 2 \in \Q[x]$. Using Eisenstein's criterion (\myref{thrm-eisenstein-criterion}) with the prime 2 tells us that $f(x)$ is irreducible over $\Q$. Since $\sqrt[6]2 \in \R$ is a zero of $f(x)$, we know that $\Q(\sqrt[6]2)$ has 6 basis vectors, so it has the underlying set
    \[
...
\begin{definition}
    Let $F$ be a field, $E/F$ be a field extension, and $f(x) \in F[x]$ be a non-constant polynomial. We say that \term{$f(x)$ splits in $E$}\index{polynomial!splits} if and only if there exist an element $a \in F$ and elements $a_1, a_2, \dots, a_n \in E$ such that
    \[
        f(x) = a(x-a_1)(x-a_2)\cdots(x-a_n).
    \]
    We call $E$ a \term{splitting field for $f(x)$ over $F$}\index{splitting field}\index{field!splitting} if and only if
    \[
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \textbf{Injective}: As $\overline{\phi}$ is non-trivial it is thus injective (\myref{thrm-homomorphism-from-field-is-injective-or-trivial}).
...
With \myref{lemma-isomorphism-extension}, we can prove the following theorem.
...
Those with a background in calculus would be familiar with the following properties of the derivative. For the following propositions, let $F$ be a field, $f(x), g(x) \in F[x]$, and $k \in F$.
...
We note an analogue of B\'ezout's lemma (\myref{lemma-bezout}) for polynomials.
...
\begin{definition}
    Let $\alpha$ be a zero of multiplicity $n$ of a polynomial $f(x)$. If $n = 1$ then $\alpha$ is a \term{simple zero}\index{polynomial!zeroes!simple}; otherwise $\alpha$ is a \term{multiple zero}\index{polynomial!zeroes!multiple}.
\end{definition}

% With the derivative, we can derive a test for multiple zeroes.
...
\end{theorem}
\begin{proof}
    For the forward direction, let $\alpha \in E$ be a multiple zero of $f(x)$. Then there is a $g(x) \in E[x]$ such that $f(x) = (x-\alpha)^2g(x)$, by the Factor Theorem (\myref{corollary-factor-theorem}). One sees that
    \begin{align*}
...
\begin{proof}
    ...
    \begin{itemize}
        \item \textbf{Injective}: As $\phi$ is non-trivial it is thus injective (\myref{thrm-homomorphism-from-field-is-injective-or-trivial}).
...
\begin{proposition}[Freshman's Dream]\label{prop-freshman-dream}\index{Freshman's Dream}
    Let $F$ be a field of prime characteristic $p$. Then $(x + y)^{p^n} = x^{p^n} + y^{p^n}$ for all $x,y \in F$ and non-negative integers $n$.
\end{proposition}
\begin{remark}
...
\end{remark}
\begin{proof}
    The case when $n = 0$ is obvious, so we induct only for $n > 0$.
...
What if the irreducible polynomial has multiple zeroes in a splitting field? In this case, there is something remarkable about the multiplicities of the zeroes.
