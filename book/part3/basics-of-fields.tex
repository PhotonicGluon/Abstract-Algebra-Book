...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item \textbf{Distributive}\index{axiom!field!distributive}: $\cdot$ distributes over $+$, i.e. $a \cdot (b + c) = (a \cdot b) + (b \cdot c)$ for all $a,b,c \in F$.
    \end{itemize}
\end{definition}
...
\end{proposition}
\begin{proof}
    Since all fields are integral domain, the result follows from \myref{prop-zero-or-prime-characteristic-if-integral-domain}.
\end{proof}
...
\begin{definition}
    Let $K$ be a subset of a field $F$. Then $K$ is a \term{subfield}\index{subfield} of $F$ if and only if $K$ is also a field under the same operations as $F$.
\end{definition}
...
\end{proposition}
\begin{proof}
    We know that the intersection of subfields of a field is itself a subfield by \myref{prop-intersection-of-subfields-is-subfield}. Thus the intersection of all the subfields of the field is also a subfield.
\end{proof}
