...
Like with groups, if $|F| = n$ where $n$ is finite, then we say $F$ is a \term{finite field}\index{field!finite}\index{finite field}. On the other hand, if $|F| = \infty$, then we say that $F$ is an \term{infinite field}\index{field!infinite}.
...
\begin{definition}
    Let $K$ be a subset of a field $F$. Then $K$ is said to be a \term{subfield}\index{subfield} of $F$ if $K$ is also a field under the same operations as $F$.
\end{definition}
