...
\begin{remark}
    We may abuse the definition of lines and line segments and call line segments ``lines''.
\end{remark}
...
\subsection{Polygons}
\begin{definition}
    A \term{polygon}\index{polygon} is a plane figure made up of \term{sides}\index{polygon!side} connected to form a closed figure. An $n$-sided polygon (where $n \geq 3$) is an \term{$n$-gon}\index{$n$-gon}.
\end{definition}
...
\begin{definition}
    A \term{right-angled triangle}\index{triangle!right-angled} is a triangle where one of its angles is a right angle. The longest side of a right-angled triangle is called its \term{hypotenuse}\index{triangle!right-angled!hypotenuse}. The other two sides are called its \term{legs}\index{triangle!right-angled!leg}.
\end{definition}
...
\begin{figure}[H]
    \centering
    \pdfteximg[9pt]{0.475\textwidth}{part3/images/geometric-constructions/trigonometry-quadrant-1.pdf_tex}
    \pdfteximg[9pt]{0.475\textwidth}{part3/images/geometric-constructions/trigonometry-quadrant-2.pdf_tex}
    \pdfteximg[9pt]{0.475\textwidth}{part3/images/geometric-constructions/trigonometry-quadrant-3.pdf_tex}
    \pdfteximg[9pt]{0.475\textwidth}{part3/images/geometric-constructions/trigonometry-quadrant-4.pdf_tex}
...
\begin{definition}
    For any real number $\theta$, define
    \begin{itemize}
        \item $\sin\theta = \sin(\theta + 2\pi)$;
        \item $\cos\theta = \cos(\theta + 2\pi)$; and
        \item $\tan\theta = \tan(\theta + 2\pi)$.
    \end{itemize}
\end{definition}
...
\end{theorem}
\begin{remark}
    $(\sin\theta)^2$ and $(\cos\theta)^2$ are more commonly written as $\sin^2\theta$ and $\cos^2\theta$. This is not to be confused with composing $\sin$ and $\cos$ twice, i.e. $\sin^2 \theta \neq \sin(\sin \theta)$ and $\cos^2 \theta \neq \cos(\cos \theta)$.
\end{remark}

\begin{theorem}\label{thrm-sine-sum-rule}
    $\sin(\alpha+\beta) = \sin\alpha\cos\beta + \cos\alpha\sin\beta$ for any $\alpha,\beta \in \R$.
\end{theorem}
\begin{corollary}
...
\begin{theorem}\label{thrm-cosine-sum-rule}
    $\cos(\alpha+\beta) = \cos\alpha\cos\beta - \sin\alpha\sin\beta$ for any $\alpha,\beta \in \R$.
\end{theorem}
\begin{corollary}
...
\constructionproof{5}{
    \begin{figure}[H]
        \centering
        \pdfteximg[8.5pt]{0.7\linewidth}{part3/images/geometric-constructions/perpendicular-bisector.pdf_tex}
    \end{figure}
}{
...
\constructionproof{5}{
    \begin{figure}[H]
        \centering
        \pdfteximg[8.5pt]{0.7\linewidth}{part3/images/geometric-constructions/parallel-line.pdf_tex}
    \end{figure}
}{
...
\constructionproof{5}{
    \begin{figure}[H]
        \centering
        \pdfteximg[8.5pt]{0.7\linewidth}{part3/images/geometric-constructions/angle-in-semicircle.pdf_tex}
    \end{figure}
}{
...
\end{theorem}
\begin{proof}
    We know that $\Con^\ast$ is non-empty since 1 is a constructible number by definition.
...
\begin{proof}[Proof (see {\cite[pp.~272-273]{judson_beezer_2022}})]
    ...
    \end{enumerate}
    Therefore the points of intersection of lines and circles within $\con$ lie within the field $\con(\sqrt\alpha)$ where $\alpha \in \con$ and $\alpha > 0$ as desired.
\end{proof}
...
The bisection of any given angle is easy for the Greeks; trisection was hard. In fact, this is one of the impossible constructions that plagued the Greeks.
...
The Greeks also wondered whether, given any cube, another cube of twice the volume could be constructed just by using a straightedge and compass. Using the tools that we have built, we can safely conclude that this task is impossible for any general cube.
