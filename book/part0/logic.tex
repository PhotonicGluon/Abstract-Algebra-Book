...
\begin{itemize}
    \item \term{Logical NOT}\index{logical operation!NOT}/\term{Negation}\index{logical operation!negation} ($\lnot$): ``$\lnot p$'' is read as ``not $p$''.
    \item \term{Logical AND}\index{logical operation!AND}/\term{Conjunction}\index{logical operation!conjunction} ($\land$): ``$p\land q$'' is read as ``$p$ and $q$''.
    \item \term{Logical OR}\index{logical operation!OR}/\term{Disjunction}\index{logical operation!disjunction} ($\lor$): ``$p\lor q$'' is read as ``$p$ or $q$''.
    \item \term{Conditional}\index{logical operation!conditional}/\term{Implication}\index{logical operation!implication} ($\implies$): ``$p \implies q$'' can be read many different ways. For example,
    \begin{itemize}
        \item $p$ implies $q$;
        \item if $p$ then $q$;
        \item $q$ if $p$;
...
We now introduce the \term{biconditional}\index{logical operation!biconditional}.
\begin{definition}
...
\begin{definition}
    Two statements $p$ and $q$ are \term{logically equivalent}\index{logically equivalent} if and only if they have identical truth values for each possible statement that $p$ and $q$ may take.
