...
\begin{lemma}[Euclid's Division Lemma]\label{lemma-euclid-division}\index{Euclid's Division Lemma}
    ...
    Here, $n$ is called the \term{dividend}\index{dividend}, $q$ is called the \term{quotient}\index{quotient}, and $r$ is called the \term{remainder}\index{remainder}.
\end{lemma}
\begin{remark}
...
\begin{definition}
    Let $m$ and $n$ be two non-zero integers. Then an integer $l$ is said to be the \term{lowest common multiple (LCM)}\index{lowest common multiple}\index{LCM} of $m$ and $n$ if $l = pm = qn$ for some integers $p$ and $q$, and that $l$ is the smallest possible \textit{positive} integer that achieves this.
