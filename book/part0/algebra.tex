...
\begin{axiom}[Commutativity of Addition]\label{axiom-addition-is-commutative}\index{axiom!arithmetic!commutative addition}
...
\begin{axiom}[Associativity of Addition]\label{axiom-addition-is-associative}\index{axiom!arithmetic!associative addition}
    For any $x, y, z \in \R$ we have $x+(y+z) = (x+y)+z$.
\end{axiom}
...
Let's look at two properties of multiplication over the real numbers.

\begin{axiom}[Commutativity of Multiplication]\label{axiom-multiplication-is-commutative}\index{axiom!arithmetic!commutative multiplication}
    For any $x, y \in \R$ we have $x \times y = y \times x$.
\end{axiom}
\begin{remark}
...
\begin{axiom}[Associativity of Multiplication]\label{axiom-multiplication-is-associative}\index{axiom!arithmetic!associative multiplication}
    For any $x, y, z \in \R$ we have $x\times(y\times z) = (x\times y)\times z$.
\end{axiom}
...
\begin{axiom}[Distributivity of Multiplication Over Addition]\label{axiom-distributivity}\index{axiom!arithmetic!distributivity}
...
\begin{example}
    ...
    \begin{align*}
        &(x+1)(x+2)\\
        &= ((x+1)x) + ((x+1)\times2) & (\myref{axiom-distributivity})\\
...
\begin{axiom}[Multiplicative Inverse]\label{axiom-reciprocal}\index{axiom!arithmetic!reciprocal}
...
\begin{example}
    Let $A = \{3, 5, 7\}$. Then
    \[
        \sum_{x \in A} x = 3 + 5 + 7 = 15.
    \]
    We also see that
    \[
        \sum_{x \in A}(3x + 5) = (3\times3 + 5) + (3\times5 + 5) + (3\times7 + 5) = 60
    \]
    and
    \[
        \sum_{x\in A}x^2 = 1^2 + 3^2 + 5^2 + 7^2 = 83.
    \]
\end{example}
...
\begin{theorem}[Binomial Theorem]\index{Bionomial Theorem}\label{thrm-binomial}
    ...
    \]
    is read as ``$n$ choose $k$'' and is known as the \term{binomial coefficient}\index{binomial coefficient}.
\end{theorem}
...
\begin{proposition}
    ${n\choose k} = {n\choose {n-k}}$ for all integers $0 \leq k \leq n$.
\end{proposition}
\begin{proof}
