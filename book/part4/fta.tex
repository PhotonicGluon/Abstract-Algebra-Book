...
With the Intermediate Value Theorem proven, we can prove the following two lemmas. These seem obvious, but we can now make them rigorous with the results that we have proven earlier.
...
\begin{proof}
    ...
    \]
    So for $x > N$ we have $f(x) > x^{2n+1} - \epsilon x^{2n+1} > 0$ and for $x < -N$ we have $f(x) < x^{2n+1} - \epsilon x^{2n+1} < 0$. Therefore, since polynomials are continuous (\myref{prop-polynomials-are-continuous}), therefore by the Intermediate Value Theorem (\myref{thrm-intermediate-value-theorem}) we see that there exists an $a \in [N, -N]$ such that $f(a) = 0$.
\end{proof}
