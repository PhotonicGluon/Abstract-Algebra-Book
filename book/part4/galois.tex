...
Recall from \myref{section-automorphism-groups} that the set of automorphisms of a group forms a group, called the group of automorphisms. We note a similar result for field automorphisms.
...
\begin{definition}
    Let $F$ be a field and $E/F$ a field extension. The group of all automorphisms of $E$ that fix $F$ elementwise is called the \term{Galois group of $E/F$}\index{Galois group} and is denoted $\Gal{E/F}$. That is,
    \[
...
\begin{example}\label{example-galois-group-of-Q-sqrt3-sqrt5-over-Q}
    ...
    \]
    so $\sigma$ fixes $\Q(\sqrt5)$. Similarly the map $\tau: \Q(\sqrt3,\sqrt5)\to\Q(\sqrt3,\sqrt5)$ where
    \[
...
\begin{definition}
    Let $F$ be a field and $E/F$ be a field extension. Two elements $\alpha,\beta \in E$ are \term{conjugate over $F$}\index{conjugate} if and only if they have the same minimal polynomial.
\end{definition}
...
\begin{proof}[Proof (cf. {\cite[Theorem 23.7]{judson_beezer_2022}})]
    ...
    When $[E:F] = 1$, we have $E = F$ (\myref{prop-finite-extension-of-degree-1-means-extension-equals-base-field}). Therefore $\Gal{E/F}$ contains only the identity automorphism and so $|\Gal{E/F}| = 1 = [E:F]$.
...
\begin{proof}
    ...
    To prove that $\Gal{E/F}$ is cyclic we need to find a generator. In particular we need to find an isomorphism in $\Gal{E/F}$ of order $k$. Consider the map $\sigma: E \to E$ where $\phi(a) = a^{p^n}$ for any $a \in E$. We show that $\sigma \in \Aut{E}$.
    \begin{itemize}
...
\begin{example}
    Remember when we claimed $\Gal{\Q(\sqrt3,\sqrt5)/\Q} \cong \Z_2^2$ in \myref{example-galois-group-of-Q-sqrt3-sqrt5-over-Q}? Certainly one sees that $H = \{\id, \sigma, \tau, \mu\}$ is a subgroup of $\Gal{\Q(\sqrt3,\sqrt5)/\Q}$. However one also sees $|H| = 4$ and
    \[
        |\Gal{\Q(\sqrt3,\sqrt5)/\Q}| = [\Q(\sqrt3,\sqrt5):\Q] = 4
    \]
...
\begin{definition}
    Let $F$ be a field and $f(x) \in F[x]$. Let $E$ be the splitting field of $f(x)$ over $F$. Then $f(x)$ is \term{separable}\index{polynomial!separable} if and only if it only has simple zeroes in $E$. In other words, we have
    \[
...
Recall also from \myref{thrm-zeroes-of-an-irreducible} that an irreducible polynomial in $F[x]$ is separable if $\Char{F} = 0$, or $\Char{F} = p$ is prime and $f(x) \neq g(x^p)$ for some $g(x) \in F[x]$.

\begin{definition}
    Let $F$ be a field. An extension $E$ is a \term{separable extension}\index{extension!separable} of $F$ if and only if every element of $E$ is a zero of a separable polynomial in $F[x]$.
\end{definition}
...
\end{proposition}
\begin{proof}
    See \myref{exercise-algebraic-extension-of-field-of-characteristic-0-is-separable} (later).
\end{proof}
...
\begin{proof}[Proof (see {\cite[Theorem 21.6]{gallian_2016}})]
    ...
    So $F(c) \subseteq F(a, b)$ and $F(a, b) \subseteq F(c)$, implying $F(a, b) = F(c)$.
\end{proof}
...
\begin{proof}
    ...
    For the case when $F$ is infinite, suppose $[E:F] = n$, which means that we may write $E = F(a_1, a_2, \dots, a_n)$ where each $a_i \in E$. We induct on $n$. We choose $n > 1$ since the case of $n = 1$ would lead to $E = F$ (\myref{prop-finite-extension-of-degree-1-means-extension-equals-base-field}).
    ...
    \begin{align*}
        ...
        &= F(a_1, a_2, \dots, a_k)(a_{k+1}) & (\myref{prop-field-generated-by-S-inductive-definition})\\
...
\begin{definition}
    Let $F$ be a field and $E/F$ be a field extension. Let $H$ be a subgroup of $\Aut{E}$. Then the \term{fixed field of $H$}\index{fixed field} is
    \[
...
\begin{proof}[Proof (cf. {\cite[Proposition 23.17]{judson_beezer_2022}})]
    ...
    \]
    Therefore $[\Fix{E}{G}:F] = 1$ which yields $\Fix{E}{G} = F$ (\myref{prop-finite-extension-of-degree-1-means-extension-equals-base-field}).
\end{proof}
...
\begin{proof}[Proof (see {\cite[Theorem 16.5.2]{artin_2011}})]
    ...
    Now suppose $h(x) \in F[x]$ has a zero of $\alpha_1$. Because $\Orb{G}{\alpha_1} = \{\alpha_1, \alpha_2, \dots, \alpha_r\}$, there exists $\sigma \in G$ such that $\sigma(\alpha_1) = \alpha_i$ for some $i = 1, 2, \dots, r$. We note that $\alpha_i$ is a zero of $h(x)$ since, if we write $h(x) = a_0 + a_1x + \cdots + a_mx^m$, we see
    \begin{align*}
        0 &= \sigma(0)\\
        &= \sigma(f(\alpha_1))\\
...
\begin{proof}[Proof (see {\cite[Theorem 16.5.4]{artin_2011}})]
    ...
    Now consider the group $G$ acting on $E$ by the group action $\sigma\cdot x = \sigma(x)$ for any $\sigma \in G$ and $x \in E$. We consider $\Stab{G}{\gamma}$. Note that because $\gamma \notin F = \Fix{E}{G}$, therefore the only element of $G$ that fixes $\gamma$ is $\id$, i.e. $\Stab{G}{\gamma} = \{\id\}$. By the Orbit-Stabilizer theorem (\myref{thrm-orbit-stabilizer}), we see
    \[
...
\begin{definition}
    Let $F$ be a field. A normal separable field extension $E/F$ is called a \term{Galois extension}\index{extension!Galois} of $F$.
\end{definition}
...
\begin{proof}[Proof (see {\cite[Theorem 23.19]{judson_beezer_2022}})]
    ...
    \begin{itemize}
        ...
        \item $\boxed{(2) \implies (3)}$ Let $E$ be the splitting field of some separable polynomial $f(x) \in F[x]$ over $F$. Then by \myref{prop-fixed-field-of-Gal-E/F-is-F} we see $F = \Fix{E}{\Gal{E/F}}$. Note that $\Gal{E/F} \leq \Aut{E}$ by \myref{prop-galois-group-is-group}, and because $|\Gal{E/F}| = [E:F]$ (\myref{thrm-order-of-galois-group-is-degree-of-field-extension}) which is finite, therefore $\Gal{E/F}$ is a finite group. Therefore $F = \Fix{E}{G}$ where $G = \Gal{E/F}$, which one sees is a finite subgroup of $\Aut{E}$.

        \item $\boxed{(3) \implies (1)}$ Suppose $F = \Fix{E}{G}$ for some finite subgroup $G$ of $\Aut{E}$. By \myref{thrm-fixed-field} we see $[E:F] = |G|$, which means that $E$ is a finite extension of $F$.
        ...
        By \myref{prop-galois-field-automorphism-permutes-zeroes-of-polynomial}, since $F$ is the set of elements of $E$ that are fixed by $G$, therefore automorphisms in $G$ permute the zeroes of $f(x)$ lying in $E$. Hence, applying the automorphisms of $G$ on the known zero $\alpha$ of $f(x)$, we can obtain distinct zeroes $\alpha_1$ (which is $\alpha$), $\alpha_2, \alpha_3, \dots, \alpha_n \in E$. Now let
        \[
...
\end{corollary}
\begin{proof}
    For the forward direction, if the extension $E/F$ is finite and Galois, then $E$ is the splitting field of some separable polynomial $f(x) \in F[x]$ over $F$ (\myref{thrm-equivalence-of-finite-galois-extension}). Certainly a separable polynomial only has simple zeroes, so by \myref{thrm-order-of-galois-group-is-degree-of-field-extension} we have $|\Gal{E/F}| = [E:F]$.

    For the reverse direction we first assume $|\Gal{E/F}| = [E:F]$. We know that $\Gal{E/F} \leq \Aut{E}$ by \myref{prop-galois-group-is-group}. Also we know that $\Gal{E/F}$ is finite because $[E:F]$ is finite, and finally we know that $F = \Fix{E}{\Gal{E/F}}$. So by \myref{thrm-equivalence-of-finite-galois-extension} we see that $E$ is a finite Galois extension of $F$.
\end{proof}
...
\begin{example}
    In \myref{example-galois-group-of-Q-sqrt3-sqrt5-over-Q} we examined the automorphisms of $\Q(\sqrt3, \sqrt5)$ that fix $\Q$. Recall $\Gal{\Q(\sqrt3,\sqrt5)/\Q} = \{\id, \sigma, \tau, \mu\}$, where
    \begin{align*}
        \sigma(a + b\sqrt3 + c\sqrt5 + d\sqrt{15}) &= a - b\sqrt3 + c\sqrt5 - d\sqrt{15},\\
    ...
    \begin{figure}[H]
        \centering
        \pdfteximg[9pt]{0.475\textwidth}{part4/images/galois/Qsqrt3-Qsqrt5-Qsqrt15.pdf_tex}
        \pdfteximg[9pt]{0.475\textwidth}{part4/images/galois/Gal-Qsqrt3-Qsqrt5.pdf_tex}
        \caption{Lattices of $\Q(\sqrt3,\sqrt5)$ and $\Gal{\Q(\sqrt3,\sqrt5)/\Q}$}
    \end{figure}
\end{example}
...
\begin{proof}[Proof (see {\cite[Theorem 16.7.1]{artin_2011}})]
    We show that the composition of the two maps in either order is the identity, and our work would be done. In other words, we want to show that $\alpha\beta: \mathcal{G} \to \mathcal{G}$ is $H \mapsto H$, and want to show that $\beta\alpha: \mathcal{F} \to \mathcal{F}$ is $K \mapsto K$.
...
\begin{corollary}\label{corollary-fundamental-theorem-of-galois-theory-is-inclusion-reversing}
    The correspondence given by the Fundamental Theorem of Galois Theory (\myref{thrm-fundamental-theorem-of-galois-theory}) reverses inclusions. That is, if $K, K' \in \mathcal{F}$ and fields and $H, H' \in \mathcal{G}$ are the corresponding subgroups, then $K \subset K'$ if and only if $H > H'$.
\end{corollary}
\begin{proof}
...
\begin{example}
    ...
    Since $G$ is of order 4, there are only two possibilities of what $G$ is -- either $G \cong \Cn{4}$ or $G \cong \mathrm{V}$, the Klein four-group (this was explored way back in \myref{problem-smallest-nonabelian-group}), which are both abelian. It is easy to find three intermediate fields of degree 2 over $\Q$, namely $\Q(\sqrt3)$, $\Q(\sqrt5)$, and $\Q(\sqrt{15})$, so $G \cong \mathrm{V}$, since $\Cn{4}$ has only one subgroup of order 2.

    The subgroups of order 2 are the only proper subgroups of $G$. Consequently one sees by \myref{corollary-intermediate-field-galois-field-order} that there are no more intermediate fields than the three we found.
\end{example}

Now suppose that we are given a chain of fields $F \subset K \subset E$, and that $E/F$ is a Galois extension. Then by \myref{prop-intermediate-field-of-galois-extension-is-galois-extension} we know that $E/K$ is a Galois extension, but $K/F$ may not be. So when is $K/F$ a Galois extension?
...
\begin{proof}[Proof (see {\cite[Theorem 23.23, Statment 4]{judson_beezer_2022}})]
    ...
    \begin{itemize}
        ...
        \item \textbf{Kernel}: The kernel of $\phi$ is the set of automorphisms of $G$ that are the identity when restricted to $K$. So since $K = \Fix{E}{H}$, thus the kernel is $H$.

        \item \textbf{Image}: The First Isomorphism Theorem (\myref{thrm-isomorphism-1}) tells us that $G/H \cong \im\phi$, which means $|\im\phi| = |G/H|$. We note that $\im\phi \leq \Gal{K/F}$. However one also sees that
        \begin{align*}
            &|\Gal{K/F}|\\
            &= [K:F] & (K/F \text{ is Galois; } \myref{corollary-galois-iff-galois-field-has-order-of-degree-of-field-extension})\\
...
\begin{definition}
    An extension field $E$ of a field $F$ is a \term{radical extension}\index{extension!radical}\index{radical extension} if and only if there exists a chain of subfields
    \[
        F = F_0 \subset F_1 \subset F_2 \subset \cdots \subset F_n = E,
    \]
    called a \term{radical series}\index{radical series}, such that for $i = 1, 2, \dots, n$ we have $F_i = F_{i-1}(\alpha_i)$ for some $\alpha_i \in E$ where $\alpha_i^{k_i} \in F_{i-1}$ for some positive integer $k_i$. A radical extension of the form $F(\alpha)$ is called a \term{simple radical extension}\index{extension!radical!simple}\index{radical extension!simple}.
\end{definition}

\begin{definition}
    Let $F$ be a field and $f(x) \in F[x]$. We say that $f(x)$ is \term{solvable by radicals over $F$}\index{solvable!by radicals} if and only if the splitting field of $f(x)$ over $F$ is contained in a radical extension of $F$.
\end{definition}
...
\begin{example}\label{example-depressed-cubic-formula}
    ...
    \begin{gather*}
        A+B,\\
        -\frac{A+B}2 + \frac{(A-B)\sqrt{-3}}{2}, \text{ and}\\
...
\begin{definition}
    The zeroes of $x^n - 1$ are called the \term{$n$th roots of unity}\index{$n$th roots of unity}.
\end{definition}

\begin{definition}
    An $n$th root of unity $\omega$ is called a \term{primitive $n$th root of unity}\index{$n$th roots of unity!primitive} if and only if $\omega^m \neq 1$ for all $m \in \{1, 2, \dots, n-1\}$.
\end{definition}
...
\begin{definition}
    We say that a group $G$ is \term{solvable}\index{solvable}\index{solvable!group}\index{group!solvable} if and only if it has a subnormal series
    \[
...
\begin{proof}
    ...
    \]
    where each composition factor $K_{i+1}/K_i$ is abelian and simple. By \myref{thrm-abelian-group-simple-iff-cylic-group-of-prime-order} this means that each composition factor must be a cyclic group of prime order.
\end{proof}

\begin{example}
    Note that the group $\Sn{4}$ is solvable. Recall from  \myref{problem-S4-composition-series} that the composition series of $\Sn{4}$ is
    \[
    ...
    \end{itemize}
    Therefore the composition factors are all cyclic groups of prime order. Hence $\Sn{4}$ is solvable by \myref{prop-solvable-equivalence-for-finite-groups}.
\end{example}
...
\begin{proof}[Proof (see {\cite[Theorem 32.2]{gallian_2016}})]
    ...
    Now because $E$ is the splitting field of $x^n - a$ over $F$, we know $E$ must be the splitting field of $x^n - a$ over $F(\omega)$. In particular, $F(\omega)$ contains a primitive $n$th root of unity. Therefore, by the first case, we know that $\Gal{E/F(\omega)}$ is abelian. By \myref{prop-intermediate-field-of-galois-extension-is-galois-extension}, $E$ is a Galois extension of $F(\omega)$. Now recall that $F(\omega)$ is a Galois extension, which means
    \[
        \Gal{F(\omega)/F} \cong \Gal{E/F}/\Gal{E/F(\omega)}
    \]
    by \myref{thrm-intermediate-field-that-is-galois-extension-of-base-field-iff-subgroup-is-normal}. That theorem also says $\Gal{E/F(\omega)} \lhd \Gal{E/F}$, which thus means that
    \[
...
We are now ready to establish the connection between polynomials that are solvable by radicals over a field and solvable groups.

\begin{theorem}[Galois]\label{thrm-solvable-by-radicals-implies-solvable-group}
    Let $F$ be a field of characteristic 0 and $f(x) \in F[x]$. Suppose the splitting field of $f(x)$ over $F$ is $E$, and that $E$ is contained in a radical extension. Then $\Gal{E/F}$ is solvable.
\end{theorem}
\begin{proof}[Proof (see {\cite[Theorem 32.5]{gallian_2016}})]
    We induct on $n$.

    When $n = 1$, we have the relation that $F \subseteq E \subseteq F(\alpha_1)$. For brevity let $a = \alpha_1^{k_1}$ and let $L$ be the splitting field of $x^{k_1} - a$ over $F$. Then we have $F \subseteq E \subseteq L$, and both $E$ and $L$ are splitting fields of polynomials over $F$, which means that both $L/F$ and $L/E$ are Galois extensions (\myref{thrm-equivalence-of-finite-galois-extension}). By \myref{thrm-intermediate-field-that-is-galois-extension-of-base-field-iff-subgroup-is-normal} we thus have
    \[
    ...
    Now suppose that the theorem holds for a particular positive integer $t$; we show that the theorem holds for $t + 1$. For brevity, let $a = \alpha_1^{k_1}$. Let $L$ be the splitting field of $x^{k_1} - a$ over $E$ and let $K \subseteq E$ be the splitting field of $x^{k_1} - a$ over $F$. Then $L$ is the splitting field of $(x^{k_1}-a)f(x)$ over $F$, and $L$ is the splitting field of $f(x)$ over $K$. Since $F(\alpha_1) \subseteq K$, thus $f(x)$ splits over $K(\alpha_2, \alpha_3, \dots, \alpha_t, \alpha_{t+1})$. Using the induction hypothesis implies that $\Gal{L/K}$ is solvable. Also \myref{thrm-galois-group-of-polynomial-of-nth-root-of-a-is-solvable} tells us that $\Gal{K/F}$ is solvable. Note that both $L/F$ and $L/K$ are Galois extensions (\myref{thrm-equivalence-of-finite-galois-extension}) and so
    \[
...
Now we are going to do something unprecedented in this book -- we will use a result without proving it first. This fact will be proven in the next (and final) chapter of the book, but for now let's assume this fact to be true.
\begin{quote}
...
\begin{proof}
    Eisenstein's Criterion (\myref{thrm-eisenstein-criterion}) with the prime 2 tells us that $f(x)$ is irreducible over $\Q$; consequently this means that $f(x)$ is separable in $F$ (\myref{thrm-zeroes-of-an-irreducible}).
...
\end{proposition}
\begin{proof}
    We first find a composition series of $\Sn{5}$. Note that $\An{5}$ is a normal subgroup of $\Sn{5}$ (\myref{prop-An-normal-subgroup-of-Sn}), $|\Sn{5}| = 5! = 120$ (\myref{exercise-order-of-Sn}), and $|\An{5}| = \frac{5!}2 = 60$ (\myref{prop-order-of-An}). Since the order of a subgroup must divide the order of the group by Lagrange's theorem (\myref{thrm-lagrange}), the maximal order of a proper subgroup of $\Sn{5}$ is 60. Hence $\An{5}$ is the maximal (normal) subgroup of $\Sn{5}$. However $\An{5}$ is a simple group (\myref{thrm-An-is-simple-for-n>=5}) which means that it has no proper normal subgroups. Hence the composition series of $\Sn{5}$ is
    \[
        1 \lhd \An{5} \lhd \Sn{5}.
    \]

    However, note that the composition factor $\An{5}/1 \cong \An{5}$ is not a cyclic group of prime order. Therefore, by \myref{prop-solvable-equivalence-for-finite-groups}, we conclude that $\Sn{5}$ is not solvable.
\end{proof}
...
\begin{problem}
    Let $\theta \in \R$. \term{De Moivre's formula}\index{De Moivre's formula} states that
    \[
...
\begin{problem}
    ...
    \]
    For example, for two distinct zeroes, we have $D = \alpha_1 - \alpha_2$. For three distinct zeroes, we have $D = (\alpha_1-\alpha_2)(\alpha_1-\alpha_3)(\alpha_2-\alpha_3)$. The \term{discriminant}\index{discriminant}\index{polynomial!discriminant} of $f(x)$ is defined to be $\Delta = D^2$.
    \begin{partquestions}{\roman*}
