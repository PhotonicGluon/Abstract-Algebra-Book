...
\begin{proof}
    ...
    Now suppose $\phi$ and $\psi$ be two automorphisms of $E$ such that $\phi(a) = a$ and $\psi(a) = a$ for all $a \in F$. Then $\phi\psi^{-1}$ is an automorphism of $E$ where
    \begin{align*}
...
\begin{proof}
    ...
    For the reverse direction we first assume $|\Gal{E/F}| = [E:F]$. We know that $\Gal{E/F} \leq \Aut{E}$ by \myref{prop-galois-group-is-group}. Also we know that $\Gal{E/F}$ is finite because $[E:F]$ is finite, and finally we know that $F = \Fix{E}{\Gal{E/F}}$. So by \myref{thrm-equivalence-of-finite-galois-extension} we have $E$ is a finite Galois extension of $F$.
\end{proof}
...
\begin{proof}
    ...
    \]
    where each composition factor $K_{i+1}/K_i$ is abelian and simple. By \myref{thrm-abelian-group-simple-iff-cylic-group-of-prime-order} we know that this means that each composition factor must be a cyclic group of prime order.
\end{proof}
