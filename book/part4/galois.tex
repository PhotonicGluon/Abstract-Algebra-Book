...
\begin{definition}
    Let $F$ be a field and $f(x) \in F[x]$. Let $E$ be the splitting field of $f(x)$ over $F$. Then $f(x)$ is called \term{separable}\index{polynomial!separable} if and only if it has only simple zeroes in $E$. In other words, we have
    \[
