...
\begin{example}
    ...
    \begin{itemize}
        \item for all $x, y \in \Z_6$ we see $x \oplus_6 y \in \Z_6$;
        \item for all $x, y, z \in \Z_6$ we see $x \oplus_6 (y \oplus_6 z) = (x \oplus_6 y) \oplus_6 z$;
...
It should be noted that, in this book, we use the convention of reading the \textit{row before the column} in a group table. However, since $(\Z_6, \oplus_6)$ is an abelian group, the order does not matter. We will look at Cayley tables of non-abelian groups later.
...
\begin{exercise}
    By using a Cayley table, show that $(\Z_6, \otimes_6)$ does \textit{not} form a group.
\end{exercise}
...
We look at the notion of the order\index{order} of a group.
...
Cyclic groups may also be of \term{infinite order}. Such cyclic groups are called \term{cyclic groups of infinite order} or \term{infinite cyclic groups}\index{cyclic group!infinite}.
\begin{example}
...
\begin{example}
    ...
    \]
    which quickly means that $D_1$ has only two elements, $e$ (which is $r$) and $s$. Thus $D_1$ is, in fact, just the cyclic group of order 2. One can verify the fact by looking at the Cayley table of $D_1$.
