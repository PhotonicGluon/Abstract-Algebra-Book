...
\begin{proof}[Proof (see {\cite[Lemma 13.8]{judson_beezer_2022}})]
    A corollary of the First Sylow Theorem (\myref{corollary-sylow-p-subgroup-exists}) tells us that each $G_i$, which is a Sylow $p_i$-subgroup, exists within $G$. Note that if $g \in G_i$ is a non-identity element with order $p_i^r$ then $|g^{-1}|$ also has order $p_i^r$ by a property of $p$-groups. Observe also that if $h \in G_i$ has order $p_i^s$, then if $t$ is the larger of $r$ and $s$ we see
    \[
