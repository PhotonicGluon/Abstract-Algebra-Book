...
\begin{example}
    Suppose we wish to classify all abelian groups of order $60 = 2^2 \times 3 \times 5$. Then the Fundamental Theorem of Finite Abelian Groups (\myref{thrm-fundamental-theorem-of-finite-abelian-groups}) tells us that there are only two possibilities, namely
    \begin{itemize}
...
We also note a stronger converse to Lagrange's theorem (\myref{thrm-lagrange}) than we could obtain by using Sylow's theorems, but which only applies to abelian groups. This does not use the Fundamental Theorem of Finite Abelian Groups, but this is an important result nonetheless.
...
\begin{proof}[Proof (see {\cite[Lemma 13.8]{judson_beezer_2022}})]
    Recall that a corollary of the First Sylow Theorem (\myref{corollary-sylow-p-subgroup-exists}) tells us that each $G_i$, which is a Sylow $p_i$-subgroup, exists within $G$. Note that, by a property of $p$-groups, if $g \in G_i$ is a non-identity element with order $p_i^r$ then $|g^{-1}|$ also has order $p_i^r$. Observe also that if $h \in G_i$ has order $p_i^s$, then
    \[
...
\begin{proof}[Proof (see {\cite[Lemma 13.9]{judson_beezer_2022}})]
    ...
    \begin{itemize}
        ...
        \item Now we show that $G = \langle g\rangle K$. Since $G$ is an abelian group, we certainly know that $\langle g\rangle K = K\langle g\rangle$, which therefore means $\langle g\rangle K \leq G$ (\myref{prop-subgroup-product-is-subgroup}). But one also sees that
        \begin{align*}
    ...
    \end{itemize}
    So $G$ is the internal direct product of $\langle g\rangle$ and $K$, which means $G \cong \langle g\rangle \times K$ by \myref{thrm-direct-product-equivalence}.
\end{proof}
