...
\begin{definition}
    Let $(G, \ast)$ and $(H, \star)$ be groups. The \term{external direct product}\index{direct product!external} of $G$ and $H$ is denoted by $G\times H$ and is the group $(G\times H, (\ast, \star))$, where $(\ast, \star)$ are the operators performed component-wise.
\end{definition}
...
\end{proposition}
\begin{proof}
    See \myref{exercise-external-direct-product-is-group} (later).
\end{proof}
...
\begin{proof}[Proof (see {\cite[Proposition 13.1 (3)]{humphreys_1996}})]
    ...
    Suppose on the contrary that $d > 1$. Take $(a, b) \in \Z_m \times \Z_n$. Note that $\frac{mn}{d} = \lcm(m,n)$ (\myref{prop-product-of-gcd-and-lcm}) which is a positive integer. Then
    \begin{align*}
    ...
    \begin{align*}
        ...
        &= mn & (\gcd(m,n) = 1).
    \end{align*}
    Since $|\Z_m \times \Z_n| = mn$ and $|(1,1)| = mn$, thus $\Z_m \times \Z_n$ is cyclic with generator $(1,1)$. By \myref{thrm-finite-cyclic-group-isomorphic-to-Zn}, $\Z_m \times \Z_n \cong \Z_{mn}$.
\end{proof}
...
\begin{definition}\label{definition-subgroup-product}
    Let $G$ be a group with subgroups $H$ and $K$. Then the \term{subgroup product of $H$ and $K$}\index{subgroup product} is
    \[
...
We now look at the internal direct product, another way of `combining' groups.
...
\begin{definition}
    ...
    \end{itemize}
    Then $G$ is said to be the \term{internal direct product}\index{direct product!internal} of $H$ and $K$.
\end{definition}
...
\begin{example}
    Consider again $G = D_6$, $H = \langle r^3 \rangle$ and $K = \langle s, r^2 \rangle$. As we have found before, $G$ is the internal direct product of $H$ and $K$; but we now know that $G \cong H \times K$ by \myref{thrm-direct-product-equivalence}.
...
\begin{problem}
    Consider the \term{Klein four-group $\mathrm{V}$}\index{Klein four-group} with presentation
    \[
