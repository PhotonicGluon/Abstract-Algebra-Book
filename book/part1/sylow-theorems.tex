...
What \myref{thrm-sylow-1} and \myref{corollary-sylow-p-subgroup-exists} gives us is a partial converse to Lagrange's theorem. Where Lagrange tells us the possible orders of subgroups of a finite group, there is no guarantee that such a subgroup of that order exists. The First Sylow Theorem guarantees that there are subgroups of certain orders for a finite group.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \left(ax^na^{-1}\right)^{-1} & (\text{by earlier result})\\
...
\begin{proof}[Proof (see {\cite[Theorem 11.10]{humphreys_1996}})]
    ...
    \begin{itemize}
        ...
        \item \textbf{Identity}: $e \cdot X = eXe^{-1} = X$ all $X \in \mathcal{X}$.
        ...
        \begin{align*}
            ...
            &= (h_1h_2) \cdot X
        \end{align*}
        for any $X \in \mathcal{X}$. Thus this is indeed a group action.
    \end{itemize}
    ...
    Furthermore, we know that $g^{-1}Hg \cong H$ by \myref{exercise-conjugate-subgroup-isomorphic-to-subgroup}, which means that $|H| = |g^{-1}Hg|$. Therefore $g^{-1}Hg$ is a Sylow $p$-subgroup of $\N{G}{H}$. Hence, by \myref{prop-normalizer-of-sylow-p-subgroup}, we see $H = g^{-1}Hg$, implying $gHg^{-1} = H$. But because $X = gHg^{-1}$, so $X = H$.
...
\begin{exercise}
    Let $G$ be a group of order 784. Let $P$ be a Sylow 7-subgroup that is not a normal subgroup of $G$. Find the order of $\N{G}{P}$.
\end{exercise}
...
\begin{example}\label{example-using-kernel-to-show-non-simple}
    ...
    \end{align*}
    We note that $\ker\phi \neq \{e\}$. This is because that would imply that $\phi$ is injective (\myref{exercise-trivial-kernel-means-injective}), which would then mean $36 = |G| \leq |\Sn{4}| = 4! = 24$, a contradiction. We also note $\ker\phi \neq G$, otherwise
    \[
