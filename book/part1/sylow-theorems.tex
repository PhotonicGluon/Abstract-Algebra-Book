...
\begin{proof}[Proof (see {\cite[Theorem 15.4]{judson_beezer_2022}})]
    ...
    For the base case, if $|G| = 1$ and $p^n$ divides $|G|$ then it must be the case that $n = 0$. So a subgroup of order $p^n = p^0 = 1$ exists (which is the trivial group). Also, if $|G| = p$ then we are done -- the group itself is a subgroup of order $p$.
...
\begin{definition}
    Let $G$ be a group and $S$ be a subset of $G$. The \term{normalizer of $S$ in $G$}\index{normalizer} is given by
    \[
...
\begin{exercise}\label{exercise-normalizer-is-subgroup-of-main-group}
    Let $G$ be a group and $S$ be a non-empty subset of $G$. Prove that $\N{G}{S} \leq G$.
\end{exercise}
...
\begin{proof}[Proof (see {\cite[Proposition 11.9]{humphreys_1996}})]
    ...
    By the Diamond Isomorphism Theorem (\myref{thrm-isomorphism-2}), statement 3, we see that $PQ \leq \N{G}{P}$ since $P \unlhd \N{G}{P}$ by \myref{prop-subgroup-is-a-normal-subgroup-of-normalizer}. As $\N{G}{P} \leq G$ thus $PQ \leq G$. In addition, by \myref{exercise-order-of-subgroup-product},
    \[
...
\begin{exercise}
    Let $G$ be a group of order 784, and let $P$ be a Sylow 7-subgroup that is \textit{not} a normal subgroup of $G$. Find the order of $\N{G}{P}$.
\end{exercise}
...
\begin{definition}
    Let $G$ be a group where $|G| \geq 2$. Then $G$ is \term{simple}\index{group!simple} if the only normal subgroups of $G$ are the trivial subgroup and $G$ itself. Equivalently, $G$ is simple if $G$ has no non-trivial proper normal subgroups.
\end{definition}
