...
\begin{questions}
    \item We note that $12 = 2^2 \times 3$. Thus a Sylow 2-subgroup must have order 4. Clearly $|3| = 4$ so $\langle 3 \rangle = \{0, 3, 6, 9\}$ is a Sylow 2-subgroup of $\Z_{12}$. (It is in fact the only one.)
    ...
    \item We know $gHg^{-1} = K$ by the Second Sylow Theorem (\myref{thrm-sylow-2}). Since $H \cong gHg^{-1}$ by \myref{exercise-conjugate-subgroup-isomorphic-to-subgroup}, thus $H \cong gHg^{-1} = K$ as required.
    ...
    \item Note $130 = 2 \times 5 \times 13$. Consider $n_{13}$, the number of Sylow 13-subgroups. The Third Sylow Theorem (\myref{thrm-sylow-3}) tells us that
    \begin{itemize}
...
\begin{questions}
    ...
    \]
    which the result follows by subtracting 1 on both sides. Hence, the only possible value that satisfies both conditions is $n_q = 1$. By a corollary of the Second Sylow Theorem (\myref{corollary-sylow-subgroup-is-normal-if-it-is-unique}) this means that the only Sylow $q$-subgroup is normal, hence showing that a group with an even perfect number order is non-simple.
    ...
    \item \begin{partquestions}{\roman*}
        ...
        \end{itemize}
        Hence, since the total number of elements is $pq$, the number of elements of order $pq$ is
        \begin{align*}
            &pq - \left((p-1)+(q-1)+1\right)\\
            &= pq - (p+q - 1)\\
            &= pq - p - q + 1\\
            &> 2q - 2 - q + 1 & (\text{since } p \geq 2)\\
            &= 2q - q - 1\\
            &= q - 1\\
            &> 0 & (\text{since } q \geq 2)
        \end{align*}
    ...
    \item \begin{partquestions}{\roman*}
        \item Let $P$ be a Sylow $p$-subgroup of $N$. Then Lagrange's theorem (\myref{thrm-lagrange}) tells us that $|G| = [G:N]|N|$. Since $p$ does not divide $[G:N]$ we must have $|N| = p^ka$ where $a$ divides $m$. Hence $|P| = p^k$ as $P$ is a Sylow $p$-subgroup of $N$. Since $P$ has order $p^k$ and $P \leq N \leq G$, thus $P$ is also a Sylow $p$-subgroup of $G$.

        \item Let $Q$ be a Sylow $p$-subgroup of $G$. The Second Sylow Theorem (\myref{thrm-sylow-2}) tells us there exists $g \in G$ such that $Q = gPg^{-1}$. Recall by definition of normality that $gNg^{-1} = N$ for any $g \in G$. Note also that $P \leq N$. Hence,
        \[
            Q = gPg^{-1} \leq gNg^{-1} = N
        \]
    ...
    \item We note $3325 = 5^2 \times 7 \times 19$. Let a group of order 3325 be $G$. We know that
    \begin{itemize}
    ...
    Now consider the group $PH$. Since 5 and 133 are coprime, thus $P \cap H = \{e\}$. In addition, since $P \lhd G$, thus $PH \leq G$ by the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}), statement 3. Also,
    \[
    ...
    \end{itemize}
    Hence $n_5 = 1$ or $n_5 = 6$. Seeking a contradiction, assume $n_5 = 6$, and let $P_5$ be a Sylow 5-subgroup.
    ...
    \end{itemize}
    Thus $n_3 = 1$ or $n_3 = 10$. Now if $n_3 = 10$ then there must be $10(3-1) = 20$ elements of order 3, a contradiction to the fact there exists only 6 elements with order not equal to 5. Hence $n_3 = 1$, meaning the only Sylow 3-subgroup (call it $P_3$) is normal in $G$.

    As $P_5 \leq G$ and $P_3 \lhd G$, thus we have $P_5P_3 \leq G$ by the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}), statement 3. Note $P_5 \cap P_3 = \{e\}$ by \myref{problem-intersection-of-coprime-subgroups}. So \myref{exercise-order-of-subgroup-product} tells us
    \[
    ...
    Now \myref{problem-group-of-order-pq-has-normal-subgroup-of-order-q} tells us there exists a unique normal subgroup of $P_5P_3$, say $H$, with $|H| = 5$. But since $P_5P_3 \lhd G$, \myref{problem-normal-subgroup-of-G-contains-all-sylow-p-subgroups} tells us that $P_5P_3$ contains all Sylow 5-subgroups of $G$, meaning $G$ has only 1 Sylow 5-subgroup, i.e. $n_5 = 1$, a contradiction to our assumption that $n_5 = 6$.
