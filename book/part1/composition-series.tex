...
\begin{definition}
    Let $G$ be a group. A \term{subgroup series without repetition}\index{subgroup series}\index{subgroup series!without repetition} of $G$ is a chain of subgroups $H_0, H_1, \dots, H_n$ such that
    \[
...
\begin{definition}
    Let $G$ be a group. A \term{subnormal series}\index{subnormal series} of $G$ is a sequence of subgroups $H_0, H_1, \dots, H_n$ such that
    \[
...
\begin{definition}
    Let $G$ be a group and $N \lhd G$. Then $N$ is said to be a \term{maximal normal subgroup}\index{maximal normal subgroup} of $G$ if there does not exist a normal subgroup $H$ such that $N < H \lhd G$.
\end{definition}
\begin{remark}
...
\end{remark}
\begin{remark}
    An equivalent definition of a composition series is that it is a subnormal series where the composition factors $H_{i+1}/H_i$ are simple, as guaranteed by \myref{thrm-maximal-normal-subgroup-iff-quotient-is-simple}.
\end{remark}

For composition series, we often write a group that is isomorphic to the subgroup rather than the actual subgroup. For example, instead of the subgroup $\{0, 3\}$ in $\Z_6$, we write $\Z_2$ or $\Cn{2}$ instead. Composition series written in this form are called composition series \term{up to isomorphism}\index{composition series!up to isomorphism}.
...
\begin{example}
    ...
    \]
    is a composition series of $\Z$. In \myref{example-subgroups-of-Z} we showed that all subgroups of $\Z$ take the form $k\Z$ where $k$ is an integer. So we may write the above composition series as
    \[
...
However, finite groups will definitely have a composition series.
...
Before we continue with the core result of this chapter, we look at Zassenhaus' lemma, which may be called the Fourth Isomorphism Theorem\index{isomorphism theorem!group!fourth}.
...
\begin{proof}[Proof (see {\cite[Lemma I.8.9]{hungerford_1980}})]
    ...
    Finally, we apply the First Isomorphism Theorem (\myref{thrm-isomorphism-1}) to both $\phi$ and $\psi$, yielding statement 3,
    \[
...
\end{theorem}
\begin{remark}
    This theorem is also called the \term{Schreier-Zassenhaus Theorem} (e.g. in {\cite[\S 72]{clark_1984}}) due to the use of Zassenhaus' lemma in the proof.
\end{remark}
\begin{proof}[Proof (see {\cite[\S 72]{clark_1984}})]
    ...
    \begin{equation}\label{eqn:thrm-schreier-eqn-a-prime}
        ...
        \tag{A'}
    \end{equation}
    such that $\hat{H}_k = H_q(H_{q+1}\cap K_r)$ for $k = qn + r$, where $0 \leq q < m$ and $0 \leq r \leq n$. It needs to be demonstrated that $\hat{H}_k$ is well-defined. Consider the ambiguity of $k = qn + r$ in the case where $r = n$, so $k = qn + n = (q+1)n + 0$. For \myref{eqn:thrm-schreier-eqn-a-prime} to be well defined, we require
    \[
    ...
    Having created series \myref{eqn:thrm-schreier-eqn-a-prime}, use the same procedure to form the series
    \begin{equation}\label{eqn:thrm-schreier-eqn-b-prime}
        ...
        \tag{B'}
    \end{equation}
    such that $\hat{K}_l = H_q(H_{q+1}\cap K_r)$ for $l = qm + r$, where $0 \leq q < n$ and $0 \leq r \leq m$. The conclusions made for \myref{eqn:thrm-schreier-eqn-a-prime} apply to \myref{eqn:thrm-schreier-eqn-b-prime}.
    ...
    \end{align*}
    with the isomorphism achieved by Zassenhaus' lemma, statement 3. So \myref{eqn:thrm-schreier-eqn-a-prime} and \myref{eqn:thrm-schreier-eqn-b-prime} have isomorphic factors.
...
\begin{example}
    ...
    \begin{itemize}
        \item $\Cn{2}/1 \cong \Cn{2}$, $\Cn{6}/\Cn{2} \cong \Cn{3}$ (\myref{exercise-Zmn-mod-Zn-cong-Zn}), and $D_6 / \Cn{6}$;
