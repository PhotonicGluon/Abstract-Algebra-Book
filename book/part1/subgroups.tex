...
\begin{definition}
    Let $G$ be a group with operation $\ast$. Let $H$ be a subset of $G$. Then $H$ is said to be a \term{subgroup}\index{subgroup} of $G$, and we write $H \leq G$, if and only if it is also a group under the same operation $\ast$.
\end{definition}
\begin{remark}
...
\begin{example}\label{example-subgroups-of-Z}
    ...
    Let $n$ be the smallest positive integer in $H$. Let $m$ be any other number in $H$. Then by Euclid's division lemma (\myref{lemma-euclid-division}), we may write $m = qn + r$ where $q, r \in \Z$ such that $0 \leq r < n$. Hence,
    \[
        r = m + \underbrace{(-n) +(-n) +(-n) + \cdots + (-n)}_{|q| \text{ times}}.
    \]
    Note that since both $m$ and $-n$ are in $H$, thus $r = m + (-n) + \cdots + (-n)$ is also in $H$, since $H$ is closed under addition. But $0 \leq r < n$ and $n$ is the smallest positive integer in $H$. Thus, $r \not> 0$ which means $r = 0$. Therefore $m = nq$, i.e. every element in $H$ is a multiple of the smallest positive integer in $H$.
...
\begin{exercise}
    ...
    \begin{partquestions}{\alph*}
        ...
        \item Prove that $G$ is a subgroup of itself.
    \end{partquestions}
\end{exercise}
\begin{remark}
    A subgroup that is not the trivial subgroup is called a \term{non-trivial subgroup}\index{subgroup!non-trivial}.
\end{remark}

We end this section by defining the idea of a proper subgroup.

\begin{definition}
    A subgroup $H$ of a group $G$ is a \term{proper subgroup}\index{subgroup!proper} if and only if $H \neq G$, and we write $H < G$.
\end{definition}
\begin{remark}
...
\end{proof}
\begin{remark}
    We usually check that $H$ is non-empty by checking if the identity of $G$ is in $H$.
\end{remark}

We look at some examples of the use of the subgroup test.
\begin{example}\label{example-center-of-group}
    Let $G$ be a group. The \term{center}\index{center} of a group $G$ is
    \[
...
\begin{example}\label{example-centralizer-of-a-subset}
    Let $G$ be a group and $S$ be a non-empty subset of $G$. The \term{centralizer}\index{centralizer} of $S$ in $G$ is
    \[
    ...
    Now suppose $x, y \in \Centralizer{G}{S}$. Let $s \in S$. Note that the condition $gs = sg$ is equivalent to the condition $sg^{-1} = g^{-1}s$. So
    \begin{align*}
    ...
    \end{align*}
    which means $(xy^{-1})s = s(xy^{-1})$ for all $s \in S$. Thus $xy^{-1} \in \Centralizer{G}{S}$ for all $x, y \in \Centralizer{G}{S}$, so $\Centralizer{G}{S} \leq G$ by the subgroup test (\myref{thrm-subgroup-test}).
\end{example}
\begin{remark}
    If the set $S$ contains a single element, say $x$, we write $\Centralizer{G}{x}$ instead of $\Centralizer{G}{\{x\}}$.
\end{remark}
...
\section{Cosets}
\begin{definition}
    Let $G$ be a group, $g \in G$, and $H$ be a subgroup of $G$.
    \begin{itemize}
...
\end{definition}
\begin{remark}
    The subgroup $H$ is both a left and right coset of $H$ in $G$. This is because the element $g$ in question is $e$, so $gH = eH = H$ and $Hg = He = H$.
\end{remark}
...
\begin{proof}
    ...
    Thus we have $(1) \implies (2) \implies (3) \implies (4) \implies (5) \implies (1)$, completing the proof.
\end{proof}
...
We note one very useful corollary of \myref{lemma-coset-equality}.
...
\end{corollary}
\begin{proof}
    We prove this corollary by showing $(1) \iff (2)$ and $(1) \iff (3)$, which implies $(2) \iff (3)$.
    \begin{itemize}
...
\end{definition}
Note that the number of left cosets equals the number of right cosets, so
\[
    [G:H] = \text{Number of left cosets} = \text{Number of right cosets}
\]
...
\begin{proof}
    ...
    \end{itemize}
    The first bullet point is clear, since the identity $e$ is in every left coset. The second bullet point was proven by \myref{exercise-intersection-of-distinct-cosets-is-empty}. We only need to show the third bullet point to prove the lemma.
...
\begin{proof}
    ...
    Note that distinct left cosets partition $G$ (\myref{lemma-left-coset-partition}), so
    \[
...
\begin{exercise}
    Let $G$ be the group $(\Z_{99}, \oplus_{99})$. It is known $H = \{0, 33, 66\}$ is a subgroup of $G$. What is the index of $H$ in $G$?
\end{exercise}
...
\begin{corollary}\label{corollary-order-of-group-multiple-of-order-of-element}
    Let $G$ be a finite group and let $g \in G$. Then $|G|$ is a multiple of $|g|$.
\end{corollary}
\begin{proof}
    We are to show that $|G| = m|g|$ for some positive integer $m$.
    \begin{itemize}
        \item When $g = e$, we see $|e| = 1$ which means $|G| = |G| \times 1 = |G||e|$.
        \item When $g \neq e$, set $|g| = n$. Consider the cyclic subgroup $\langle g \rangle = \{g, g^2, g^3, \dots, g^n\}$. Note that $|g| = |\langle g \rangle| = n$, so by Lagrange's theorem (\myref{thrm-lagrange}) we have $|G| = [G:\langle g \rangle]|\langle g \rangle| = [G:\langle g \rangle]|g|$. Hence, in this case, $m = [G:\langle g \rangle]$.
    \end{itemize}
...
\begin{exercise}\label{exercise-prime-order-element}
    Prove \myref{corollary-prime-order-element}.
\end{exercise}
...
\begin{definition}
    Let $G$ be a group and $N \leq G$. We say that $N$ is a \term{normal subgroup}\index{subgroup!normal} of $G$ if and only if $gN = Ng$ for all $g \in G$, and we write $N \unlhd G$.
\end{definition}

Note that $gN = Ng$ is equivalent to the following.
\begin{itemize}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= h
    \end{align*}
    which is an element of $H$. Thus $ghg^{-1} \in H$ for all $g \in G$ and $h \in H$, meaning $H \unlhd G$.
\end{proof}
...
\begin{example}\label{example-normal-subgroups-of-d3}
    Let's find the normal subgroups of $D_3$, the dihedral group of order 6.
...
\end{definition}
\begin{remark}
    We may sometimes write $G/H$ using fractions (i.e. $\frac GH$) if it improve readability.
\end{remark}

The set of left cosets can become a special kind of group, called a quotient group, under a special operation.
...
\end{definition}
\begin{remark}
    We usually suppress the operation $\star$ and just write $(xN)(yN) = (xy)N$.
\end{remark}

\begin{theorem}\label{thrm-quotient-group-requirement}
    The group $G/N$ defined in \myref{definition-quotient-group} is indeed a group.
\end{theorem}
\begin{proof}
...
\begin{problem}
    Let $G$ be an abelian group and $H \leq G$. Define the set
    \[
