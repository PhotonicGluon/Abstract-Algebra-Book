...
\end{corollary}
\begin{proof}
    Assume that $H, K \lhd G$. By the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}), statement 3, we know that $HK \leq G$ since $H \lhd G$. We just need to prove normality. Suppose $hk \in HK$ and take $g \in G$. Then
    \begin{align*}
        g(hk)g^{-1} &= (gh)(kg^{-1})\\
...
\begin{example}
    We will prove that $\lcm(m,n)\times\gcd(m,n) = mn$ (\myref{prop-product-of-gcd-and-lcm}) by considering the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}). For brevity, let $d = \gcd(m,n)$ and $l = \lcm(m,n)$.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        By the First Isomorphism Theorem (\myref{thrm-isomorphism-1}), we have $\frac{G/N}{\ker\phi} \cong \im\phi$, which means
        \[
