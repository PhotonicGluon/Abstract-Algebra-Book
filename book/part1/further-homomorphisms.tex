...
Earlier, we introduced homomorphisms and isomorphisms, special types of maps that transform elements of one group to another. We look at more properties of such maps in this chapter and describe the uses of these new properties.

\section{Image of a Homomorphism}
\begin{definition}
    Let $G$ and $H$ be groups. The \term{image}\index{homomorphism!image} (or \term{range}\index{homomorphism!range}) of a homomorphism $\phi: G \to H$ is the set
    \[
        \im\phi = \{\phi(g) \vert g \in G\}.
    \]
\end{definition}
\begin{remark}
    Some authors (e.g. \cite[p.~6]{judson_beezer_2022}) use $\phi(G)$ to denote the image of $\phi$. The alternate notation $\mathrm{Im}\;\phi$ may also be used (e.g. by \cite[Definition I.2.2]{hungerford_1980} and \cite[\S 66]{clark_1984}).
\end{remark}
...
\begin{definition}
    Let $G$ and $H$ be groups. The \term{kernel}\index{homomorphism!kernel} of a homomorphism $\phi: G \to H$ is the set of elements in $G$ which map to the identity of $H$. That is, if the identity of $H$ is $e_H$, then the kernel of $\phi$ is the set
    \[
...
\begin{theorem}[Isomorphism I]\label{thrm-isomorphism-1}\index{First Isomorphism Theorem}\index{isomorphism theorem!group!first}
...
\begin{figure}[h]
    \centering
    \pdfteximgframed[11pt]{0.35\textwidth}{part1/images/further-homomorphisms/iso-1-commutativity.pdf_tex}
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \end{align*}
    \end{itemize}
    Thus $G/H \cong R$ by the First Isomorphism Theorem (\myref{thrm-isomorphism-1}).
\end{example}

\begin{exercise}
    Let $\phi: G \to H$ be a homomorphism between finite groups $G$ and $H$. Prove that $|G| = |\im \phi|\times|\ker \phi|$.
\end{exercise}
...
\begin{theorem}[Isomorphism II]\label{thrm-isomorphism-2}\index{Second Isomorphism Theorem}\index{isomorphism theorem!group!second}
...
We can capture the overall relationships of the subgroups of $G$ using a \term{subgroup lattice}.
...
We only show subgroups that we care about in the figure above. The group $G$ has a (direct) subgroup $HK$; $HK$ has subgroups $H$ and $K$; and $H$ and $K$ have a common subgroup $H\cap K$. The dotted quotient groups are isomorphic to each other if $H \unlhd G$.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        By the First Isomorphism Theorem (\myref{thrm-isomorphism-1}),
        \[
...
\end{corollary}
\begin{proof}
    Assume that $H$ and $K$ are proper normal subgroups of $G$. By the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}), statement 3, we know that $HK \leq G$ since $H \lhd G$. We just need to prove normality. Suppose $hk \in HK$ and take $g \in G$. Then
    \begin{align*}
        g(hk)g^{-1} &= (gh)(kg^{-1})\\
...
\begin{example}
    We say a group $G$ is \term{metabelian}\index{metabelian} if and only if there exists $A \unlhd G$ such that $A$ and $G/A$ are both abelian. We prove that any subgroup of a metabelian group is also metabelian.

    Let $H \leq G$. Then, by the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}), we know $H \cap A \unlhd H$ (statement 4) and $H/(H \cap A) \cong HA / A$ (statement 6). We just need to prove that both $H \cap A$ and $H/(H \cap A)$ are abelian.
    \begin{itemize}
...
\begin{example}
    We will prove $\lcm(m,n)\times\gcd(m,n) = mn$ (\myref{prop-product-of-gcd-and-lcm}) by using the Second Isomorphism Theorem (\myref{thrm-isomorphism-2}). For brevity, let $d = \gcd(m,n)$ and $l = \lcm(m,n)$.
    ...
    We claim that $m\Z / (l\Z) \cong \Z_{\frac lm} \text{ and } d\Z / (n\Z) \cong \Z_{\frac nd}$. This is a specific case of \myref{problem-mZ/nZ-isomorphic-to-Zn/m} (later). Hence,
    \[
...
\begin{theorem}[Isomorphism III]\label{thrm-isomorphism-3}\index{Third Isomorphism Theorem}\index{isomorphism theorem!group!third}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Clearly $N = eN \in H/N$. Let $h_1N, h_2N \in H/N$. Observe that $(h_2N)^{-1} = \left(h_2^{-1}\right)N$ by the group operation of cosets. Hence
        \[
        ...
        By the First Isomorphism Theorem (\myref{thrm-isomorphism-1}), we know that $\frac{G/N}{\ker\phi} \cong \im\phi$, which means
        \[
...
\begin{example}
    ...
    \[
        \Z_{mn}/\Z_n \cong \Z_m
    \]
    where the relation $m\Z/(mn\Z) \cong \Z_n$ is a special case of \myref{problem-mZ/nZ-isomorphic-to-Zn/m} (later).
\end{example}
...
\begin{problem}
    Let the group $R = (\R, +)$. Also let the groups $G = R^2$ and $H = \left\{(r\times\sqrt2, r\times\sqrt3) \vert r\in R\right\}$ be under component-wise addition. Prove that $G/H \cong R$.
\end{problem}
