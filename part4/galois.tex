...
\begin{definition}
    Let $F$ be a field and $f(x) \in F[x]$. We say that $f(x)$ is \textbf{solvable by radicals over $F$}\index{solvable!by radicals} if and only if $f(x)$ splits in some extension $F(\alpha_1, \alpha_2, \dots, \alpha_n)$ of $F$ and there exist positive integers $k_1, k_2, \dots, k_n$ such that
    \begin{itemize}
        \item $\alpha_1^{k_1} \in F$; and
        \item $\alpha_i^{k_i} \in F(\alpha_1, \alpha_2, \dots, \alpha_{i-1})$ for $1 < i \leq n$.
    \end{itemize}
\end{definition}

In other words, a polynomial $f(x)$ is solvable over a field $F$ if we can obtain all its zeroes by adjoining $n$th roots (for various values of $n$) into some extension of $F$. In such cases, a zero of $f(x)$ would be written as a combination of addition, subtraction, multiplication, division, and taking $n$th roots.
...
\begin{theorem}\label{thrm-solvable-by-radicals-implies-solvable-group}
    Let $F$ be a field of characteristic 0 and $f(x) \in F[x]$. Suppose $f(x)$ splits over $F(\alpha_1, \alpha_2, \dots, \alpha_n)$ where positive integers $k_1, k_2, \dots, k_n$ exist such that
    \begin{itemize}
        \item $\alpha_1^{k_1} \in F$; and
        \item $\alpha_i^{k_i} \in F(\alpha_1, \alpha_2, \dots, \alpha_{i-1})$ for $1 < i \leq n$.
    \end{itemize}
    Let $E$ be the splitting field of $f(x)$ over $F$, where $E$ is contained in $F(\alpha_1, \alpha_2, \dots, \alpha_n)$. Then $\Gal{E/F}$ is solvable.
\end{theorem}
