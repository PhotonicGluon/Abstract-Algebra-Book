...
%TODO: Rewrite below using new definition for rings
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \item Clearly multiplying two rational numbers together produce a rational number, so $\Q$ is closed under multiplication.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \textbf{Multiplication-Closure}: In Volume I we showed that matrix multiplication produces another $n \times n$ matrix. Furthermore the entries of the new matrix are elements of $R$. Hence $\Mn{n}{R}$ is closed under matrix multiplication.
...
\begin{definition}
    A \textbf{division ring}\index{division ring} $R$ is a non-trivial ring such that every element $x$ in $R$ has a multiplicative inverse $x^{-1}$ where
    \[
        xx^{-1} = x^{-1}x = 1.    
    \]
\end{definition}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item $(\{0\}, +)$ is an abelian group by Volume I.
        \item $(\{0\}, \times)$ is an abelian group by Volume I.
