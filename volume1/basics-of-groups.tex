...
\begin{axiom}\label{axiom-addition-is-associative-and-commutative}\index{axiom!of associative addition}\index{axiom!of commutative addition}
    Addition over the integers, the rational numbers, and the real numbers is assumed to be associative and commutative.
\end{axiom}
It is possible to prove that addition is both associative and commutative, but their proof is too involved to be reproduced here. As such, we leave it as an axiom.
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Associativity}: By \myref{axiom-addition-is-associative-and-commutative} we know for all integers $a$, $b$, and $c$ that $a + (b + c) = (a + b) + c$.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Associativity}: For $a$, $b$, and $c$ in $\mathbb{Z}_n$, since addition is associative (\myref{axiom-addition-is-associative-and-commutative}), thus $a \oplus_n (b \oplus_n c) = (a + (b + c)) \mod{n} = ((a + b) + c) \mod{n} = (a \oplus_n b) \oplus_n c$.
...
\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=5.5cm]{basics-of-groups/d3.jpg}}
...
\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=5.5cm]{basics-of-groups/d4.jpg}}
...
\begin{remark}
    Some authors (e.g. \cite{humphreys_1996}) will write the reflections of $D_n$ with $s$ leading $r$, i.e. $s, sr, sr^2, sr^3, \dots, sr^{n-1}$. The underlying definition, however, remains the same in either case. This fact will become evident with the proof of \myref{prop-Dn-cannonical-form} (later).
\end{remark}
...
\begin{definition}
    The \textbf{dihedral group of degree $n$}\index{dihedral group!of degree $n$} (or the \textbf{dihedral group of order $2n$}\index{dihedral group!of order $2n$}) is denoted $D_n$ and is the group with the presentation
    \[
