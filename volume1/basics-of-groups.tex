...
\end{definition}
\begin{proposition}
    The set $\mathbb{Z}_n$ with the operation $\oplus_n$ forms an abelian group, where $\oplus_n$ is addition modulo $n$. That is, $a \oplus_n b = (a + b) \mod{n}$.
\end{proposition}
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Associativity}: For $a$, $b$, and $c$ in $\mathbb{Z}_n$, since addition is associative (\myref{axiom-addition-is-associative}), thus $a \oplus_n (b \oplus_n c) = (a + (b + c)) \mod{n} = ((a + b) + c) \mod{n} = (a \oplus_n b) \oplus_n c$.
        ...
        \end{itemize}
    \end{enumerate}
    Since the four group axioms are satisfied, this is a group. Furthermore, as addition is commutative (\myref{axiom-addition-is-commutative}), thus addition modulo $n$ is commutative. Therefore $(\mathbb{Z}_n, \oplus_n)$ is a commutative group.
\end{proof}
\begin{remark}
