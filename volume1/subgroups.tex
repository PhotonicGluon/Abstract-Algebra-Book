...
\begin{theorem}[Subgroup Test]\label{thrm-subgroup-test}
    Let $G$ be a group with identity $e$, and let $H$ be a subset of $G$. Then $H \leq G$ if and only if $e \in H$ and $xy^{-1} \in H$ for all $x, y \in H$.
\end{theorem}
\begin{proof}[Proof (cf. {\cite[Proposition 4.2]{humphreys_1996}})]
    We prove the forward direction first. Suppose $H \leq G$, so $e \in H$ by definition of subgroup. Now let $x$ and $y$ be elements in $H$. Since $H$ is a subgroup of $G$, inverses exist. Thus $y^{-1}$ is in $H$. Also, since $H$ is a subgroup of $G$, thus $H$ is closed under the group operation. Hence, $xy^{-1}$ is in $H$. Therefore, if $H \leq G$ then $e \in H$ and $xy^{-1} \in H$ for all $x, y \in H$.

    We now prove the reverse direction. Suppose $e \in H$ and $xy^{-1}\in H$ for all $x$ and $y$ in $H$.
    \begin{itemize}
        \item The identity of $G$ is present in $H$ by assumption.
    ...
    \end{itemize}
    Therefore if $e \in H$ and $xy^{-1}$ is in $H$ for all $x$ and $y$ in $H$, then $H \leq G$.
