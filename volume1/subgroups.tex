...
\end{proof}
\begin{remark}
    This corollary is very useful, and arguably more useful than Coset Equality (\myref{lemma-coset-equality}), as we often consider cases where we see a coset equalling the subgroup itself. In the next two chapters, we will still cite this corollary when justifying why $xH = H$ (or $Hx = H$) means $x \in H$, but in later chapters we will not.
\end{remark}
