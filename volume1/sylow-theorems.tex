...
\begin{proof}[Proof (see {\cite[pp.~1--3]{mann_2011}})]
    ...
    The first case is when order of the center of $G$ is a multiple of $p$, i.e. $|\Z{G}| = ap$ for some positive integer $a$. By Cauchy's Theorem (\myref{thrm-cauchy}), $\Z{G}$ contains a subgroup of order $p$, say $N$. We note that $N \unlhd G$ since for all $n \in N$ and $g \in G$,
    \begin{align*}
        gng^{-1} &= (gn)g^{-1}\\
        &= (ng)g^{-1} & (\text{since n} \in N \subseteq \Z{G})\\
        &= n(gg^{-1})\\
        &= n\\
        &\in N
    \end{align*}
    ...
    We now construct $S = \{g \in G \vert gN \in \bar{S}\}$; we show that $S$ is a subgroup of $G$ under the group operation of $G$. Clearly $e \in S$ as $eN = N \in \bar{S}$. Also for any $g_1, g_2 \in S$ (which means $g_1N, g_2N \in \bar{S}$), we see that
    \[
