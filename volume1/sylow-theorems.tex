...
\end{definition}
\begin{remark}
    Equivalently, $G$ is simple if $G$ has no non-trivial proper normal subgroups.
\end{remark}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item Now suppose $G$ is non-abelian. By \myref{example-group-with-prime-power-order-has-non-trivial-center}, $G$ has a non-trivial center. Furthermore, $G \neq \Z{G}$ because $G$ is non-abelian (\myref{problem-center-of-G}). Since the center is a normal subgroup of $G$, it is thus a non-trivial proper normal subgroup of $G$, meaning $G$ is non-simple.
    \end{itemize}
