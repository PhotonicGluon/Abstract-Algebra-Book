...
\begin{example}
    We know that $\Z$ and $\Q$ are rings, and clearly $\Z \subset \Q$. Hence $\Z$ is a subring of $\Q$. Similarly, $\Q$ is a subring of $\R$, and $\R$ is a subring of $\C$.
\end{example}
...
\begin{example}
    ...
    \]
    where $i$ is the imaginary unit (with $i^2 = -1$). Clearly $\Z[i] \subset \C$; we will show that $\Z[i]$ is a subring of $\C$.
