...
The base case does not necessarily begin with $n = 1$, but sometimes with $n = 0$, and possibly with any fixed natural number $n = N$, establishing the truth of the statement for all natural numbers $n \geq N$.
...
\begin{enumerate}
    ...
    \begin{align*}
        f(k+1) &= 3f(k) - 2f(k-1) + 1\\
        &= 3(2^k - k - 1) - 2(2^{k-1} - (k-1) - 1) + 1 & \text{(by hypothesis)}\\
