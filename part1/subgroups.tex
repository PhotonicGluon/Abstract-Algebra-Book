...
\begin{example}\label{example-centralizer-of-a-subset}
    ...
    \[
        \Centralizer{C}{G}{S} = \left\{g \in G \vert \text{for all } s \in S \text{ we have } gs = sg\right\}.
    \]
    Note $\Centralizer{C}{G}{S}$ is a subset of $G$. We claim that $\Centralizer{C}{G}{S}$ is a subgroup of $G$.
    \begin{proof}
        We show that for all elements $x$ and $y$ in $\Centralizer{C}{G}{S}$, $xy^{-1}$ is also in the centralizer. Note that the condition $gs = sg$ is equivalent to the condition $sg^{-1} = g^{-1}s$.

        Clearly for all $s \in S$ we have $es = se$, so $e \in \Centralizer{C}{G}{S}$. This means that $\Centralizer{C}{G}{S}$ is non-empty.
        
        Now suppose $x$ and $y$ are in $\Centralizer{C}{G}{S}$. Let $s$ be in $S$. Then
        \begin{align*}
        ...
        \end{align*}
        so $(xy^{-1})s = s(xy^{-1})$ for all $s$ in $S$. Thus, $xy^{-1}$ is in $\Centralizer{C}{G}{S}$ for all $x$ and $y$ in $\Centralizer{C}{G}{S}$, so by subgroup test $\Centralizer{C}{G}{S} \leq G$.
    \end{proof}
\end{example}
\begin{remark}
    In the case where the set $S$ contains a single element, say $S = \{x\}$, we write $\Centralizer{G}{x}$.
\end{remark}
...
\begin{exercise}
    Let $G$ be a group, and let $g \in G$. Show that $\langle g \rangle = \{g^k \vert k \in \mathbb{Z}\}$ is a subgroup of $G$, called the \textbf{cyclic subgroup}\index{subgroup!cyclic} generated by $g$.
\end{exercise}
