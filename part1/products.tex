...
\begin{example}
    Let $\mathcal{S} = \R\setminus\{0\}$ be a group under multiplication. Then $\mathcal{S} \times \Z_3 = \{ (x, n) \vert x \in \mathcal{S} \text{ and } n \in \Z_3\}$.

    So, $(3, 2)^{-1} = \left(\frac13, 1\right)$ since
    \begin{align*}
        (3, 2)\left(\frac13, 1\right) &= \left(3 \times \frac13, 2 \oplus_3 1\right)\\
        &= (1, 0)\\
        &= (\text{Identity in }\mathcal{S}, \text{Identity in }\Z_3)
    \end{align*}
\end{example}
...
\begin{proof}[Proof (see {\cite[Proposition 13.1 (3)]{humphreys_1996}})]
    ...
    \end{align*}
    which implies $|(a, b)| \leq \frac{mn}{d} < mn$ for all $(a, b) \in \Z_m \times \Z_n$. Hence, this means that $\Z_m \times \Z_n$ is \textit{not} cyclic, since $|\Z_m \times \Z_n| = mn$ and no element in $\Z_m \times \Z_n$ has order $mn$. However, $\Z_m \times \Z_n \cong \Z_{mn}$ is cyclic, a clear contradiction. Hence, $d \not>1$ which means $d = 1$, so $\gcd(m,n) = 1$.
...
\begin{exercise}
    Find all integers $m$ and $n$ with $1 < m < n$ such that $\Z_m \times \Z_n \cong \Z_{180}$.
\end{exercise}
...
\begin{example}
    ...
    Note that $|H| = 2$ so $H \cong \Z_2$ and $K \cong D_3$ (we leave the latter as an exercise for the reader to prove). Thus,
    \[
...
\begin{problem}
    Let $G = \Z_6$, and let the subgroups $H = \{0, 2, 4\}$ and $K = \{0, 3\}$. Determine whether $G$ is the internal direct product of $H$ and $K$.
\end{problem}
