...
\begin{example}
    Let $G = \Z^2$ be a group under component-wise addition and $H = \Z$ be a group under addition. Let the homomorphism $\phi: G \to H, (a, b) \mapsto a+b$. Then, $(a, b) \in \ker\phi$ if $\phi((a,b)) = 0$, i.e. $a+b = 0$, implying $ b = -a$. Hence the kernel of $\phi$ is $\{(a, -a) \vert a \in \Z\}$.
\end{example}
...
\begin{example}
    Let $R = \{x \in \R \vert x > 0\}$, $G = \{x \in \R \vert x \neq 0\}$, and $H = \{1, -1\}$ be groups under regular multiplication. We show $G / H \cong R$.
