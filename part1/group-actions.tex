...
We can now look at the \textbf{stabilizer} of an element in $X$.
...
\begin{example}
    ...
    \begin{figure}[h]
	    \centering
        \pdfteximgframed[12pt]{0.275\textwidth}{part1/images/group-actions/labelled-square.pdf_tex}
...
We note one immediate corollary of Cauchy's Theorem.
\begin{corollary}\label{corollary-cauchy-theorem-with-subgroup}
    Let $G$ be a finite group such that $|G| = np$ where $p$ is a prime and $n$ is a positive integer. Then $G$ contains an subgroup of order $p$.
\end{corollary}
\begin{proof}
    See \myref{exercise-group-of-order-multiple-of-prime-has-subgroup-of-prime-order} (later).
\end{proof}

\begin{exercise}\label{exercise-group-of-order-multiple-of-prime-has-subgroup-of-prime-order}
    For a finite group $G$ with order $np$ where $p$ is prime and $n$ is positive, show that $G$ has a subgroup of order $p$.
\end{exercise}
