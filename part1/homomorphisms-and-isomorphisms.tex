...
\begin{example}
    Let $G$ be any group. Take $g$ from $G$. Let $\phi: \Z \to G$ (where $\Z$ is the additive group of integers) be such that $\phi(n) = g^n$ for all integers $n$. Then $\phi$ is a homomorphism, since
    \[
...
\begin{example}
    Let $\R^\times$ denote the set of non-zero real numbers and $\R_{>0}$ denote the set of positive real numbers.

    Let $\R^\times$ and $\R_{>0}$ be groups under regular multiplication. Define $f: \R^\times \to \R_{>0}, x \mapsto |x|$ where $|x|$ represents the absolute value of $x$. Then $f$ is a homomorphism as
    \[
        f(xy) = |xy| = |x||y| = f(x)f(y).
    \]
\end{example}
...
\begin{example}
    ...
    \begin{itemize}
        ...
        \begin{figure}[h]
            \centering
            \fbox{\includegraphics[width=2.5cm]{homomorphisms-and-isomorphisms/isomorphism-function-map.jpg}}
        \end{figure}
...
\begin{example}
    Let $S$ denote the set of positive real numbers. We show that $(\R, +) \cong (S, \times)$ by considering the map $\phi: \R \to S, x \mapsto e^x$.
    \begin{itemize}
        ...
        \item \textbf{Injective}: Suppose $x$ and $y$ are elements in $\R$ such that $\phi(x) = \phi(y)$. Therefore $e^x = e^y$ which quickly implies $x = y$ by applying the natural logarithm ($\ln$) on both sides.
