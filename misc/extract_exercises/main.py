...
AUTOGENERATED_COMMENT = """% THIS DOCUMENT WAS AUTOMATICALLY GENERATED BY THE `extract_questions` PACKAGE.
% DO NOT EDIT!
"""
...
def get_chapter_files(part_num, chapter_id):
    chapter_text = read_latex_file(f"{RELATIVE_PATH}/part{part_num}/{chapter_id}.tex")
    solutions_text = read_latex_file(f"{RELATIVE_PATH}/part{part_num}/question-solutions/{chapter_id}.tex")

    return chapter_text, solutions_text

def extract_questions_and_solutions(ex_or_prob, chapter_text, solutions_text):
    ...
    search_start = solutions_text.find(ex_or_prob.title() + "s")

    questions = []
    for match in chapter_pattern.finditer(chapter_text):
        questions.append(match.group("content"))

    solutions = []
    for match in solutions_pattern.finditer(solutions_text, search_start):
        solutions.append(match.group("answer"))

    return questions, solutions


def write_questions_and_solutions(questions, solutions):
    output = ""
    num = len(questions)
...
def write_chapter(part_num, chapter_id):
    ...
    for ex_or_prob in ["exercise", "problem"]:
        ...
        chapter_text, solutions_text = get_chapter_files(part_num, chapter_id)
        questions, solutions = extract_questions_and_solutions(ex_or_prob, chapter_text, solutions_text)
        output += write_questions_and_solutions(questions, solutions)
...
def generate_questions_and_solutions(book_path):
    # Get all chapters
...
code_to_add = generate_questions_and_solutions(f"{RELATIVE_PATH}/book.tex")
...
with open(f"{RELATIVE_PATH}/questions-only.tex", "w") as f:
    f.write(content)
