...
\begin{example}
    ...
    \begin{proof}
        ...
        \end{align*}
        Note that the binomial coefficients ${p \choose k}$ where $1 \leq k \leq p -1$ are all multiples of $p$ (\myref{prop-binomial-coefficient-multiple-of-n}). As the characteristic of the ring $R$ is $p$, thus $px = 0$ for any $x \in R$. Therefore,
        \begin{align*}
...
\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=4.5cm]{ring-homomorphisms/iso-1-comm-diagram.png}}
...
\begin{example}
    ...
    \[
        \left\{\begin{pmatrix}0&n\\0&0\end{pmatrix} \vert n \in \Z\right\}
    \]
    which, for brevity, we shall denote by $I$. Thus we see via the FRIT (\myref{thrm-ring-isomorphism-1}) that $R/I \cong \Z^2$.
\end{example}
\begin{exercise}
