...
\begin{definition}
    A \textbf{function}\index{function} (or a \textbf{map}\index{map}) $f$ from a set $X$ to a set $Y$, denoted $f: X \to Y$,  is a relation $f \subseteq X \times Y$ such that for every $x \in X$, there is exactly one $y \in Y$ where $x\mathrel{f}y$.
\end{definition}
\begin{remark}
...
\end{example}
\begin{example}
    Consider the function $g: \Z \to \Z, n \mapsto |n|$, where $|n|$ denotes the absolute value of $n$.
    \begin{itemize}
...
\begin{example}
    ...
    \end{itemize}
    The image/range of the function $h$ is the set
    \[
        \left\{\frac n3 \vert n \in \Z\right\},
    \]
    that is, $\im h$ is the set of all integers divided by 3.
\end{example}
