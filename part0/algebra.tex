...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item If $x < 0$, then $-x > 0$. Thus then $|x| = -x > 0$.
    \end{itemize}
    In any case, $|x| \geq 0$.
\end{proof}
...
\begin{example}
    We solve the equation $x^2 - 2|x| - 3 = 0$. By above proposition we replace $x^2$ with $|x|^2$ to obtain $|x|^2 - 2|x| - 3 = 0$, i.e. $(|x|+1)(|x|-3) = 0$. Thus $|x| = -1$ or $|x| = 3$. Now $|x| = -1$ is impossible, so $|x| = 3$, meaning $x = 3$ or $x = -3$.
\end{example}
